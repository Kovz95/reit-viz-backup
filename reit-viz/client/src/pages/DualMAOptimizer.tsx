...
// Grid-search kernel + types now live in lib/dualMaSweep.ts, shared with
// workers/dualMaSweep.worker.ts.
import {
  SLOPE_METHODS,
  GRID_CONFIGS,
  countGridCombos,
...
export default function DualMAOptimizer() {
  ...
  const handleRun = useCallback(async () => {
    ...
    // Grid searches run in Web Workers (HARSI/Oscillators pattern); slots keep
    // the results table in input order regardless of completion order.
    const sweepPool = createSweepPool(() =>
      new Worker(new URL("../workers/dualMaSweep.worker.ts", import.meta.url), { type: "module" }));
    ...
    const runItem = async (i: number) => {
      ...
      try {
        ...
        if (isPair) {
          ...
          if (!ohlc || ohlc.closes.length < 50) return;
          prices = ohlc.closes;
          ...
          if (!ws || ws.closes.length < 50) return;
          prices = ws.closes;
          priceDates = ws.dates;
        } else {
          const raw = await fetchTickerOHLCV(item.ticker);
          if (!raw || raw.adjCloses.length < 50) return;
        ...
        if (series.length < (frequency.startsWith("monthly") ? 24 : 50)) return;

        const s = series;
        const bpy = barsPerYearFor(isPair ? "daily" : frequency);
        const topResults = (await sweepPool.run<ParamResult[]>(
          { type: "run", prices: s, cfg: gridCfg, topK, barsPerYear: bpy },
          () => runDualMaSweep(s, gridCfg, topK, bpy),
        )) ?? [];
        if (topResults.length === 0) return;
        slots[i] = { ticker: item.ticker, name: (item as any).name ?? item.ticker, topK: topResults };
      } catch {} finally {
        completed++;
