// Event Lab — Study Family. Faithful port of pages/PriceAction.tsx rewired
// onto the shared event-study libs (lib/eventStudy + lib/eventSources) and
// extended with calendar conditions (earnings / ex-div / macro prints /
...
function ConditionEditor({ cond, onChange, onRemove, canRemove }: {
  cond: StudyCondition; onChange: (c: StudyCondition) => void; onRemove: () => void; canRemove: boolean;
}) {
  ...
  return (
    <div className="flex flex-wrap items-end gap-2 p-2 bg-muted/40 rounded border border-border">
      <div className="flex flex-col gap-1 min-w-[150px]">
        ...
        <Select value={cond.type} onValueChange={v => set("type", v as StudyConditionType)}>
          <SelectTrigger className="h-8 min-w-[190px] shrink-0" data-testid="select-condition-type"><SelectValue /></SelectTrigger>
          <SelectContent>
            ...
            <SelectItem value="ex_dividend">Ex-dividend</SelectItem>
      ...
      {cond.type === "month" && (
        <div className="flex flex-col gap-1 w-[120px]">
          ...
          <Select value={String(cond.month)} onValueChange={v => set("month", parseInt(v) || 1)}>
            <SelectTrigger className="h-8" data-testid="select-month"><SelectValue /></SelectTrigger>
            <SelectContent>
      ...
      {cond.type === "window" && (
        <div className="flex flex-col gap-1 w-[110px]">
          <Label className="text-xs text-muted-foreground">Start (MM-DD)</Label>
          <Input value={cond.startMMDD} placeholder="MM-DD"
      ...
      <div className="flex flex-col gap-1 w-[80px]">
        ...
        <Input type="number" step="1" min="0" value={cond.withinBars}
          ...
          title={'Condition also counts if it fired within N prior bars — enables e.g. "2σ move within 5 bars of earnings"'} />
      </div>
...
export default function StudyFamily({ preset }: { preset?: StudyPreset | null }) {
  ...
  const runCrossStudy = useCallback(async (cfg: { tickers: Array<{ ticker: string; name: string }>; conditions: StudyCondition[]; combinator: "AND" | "OR" }) => {
    ...
    const needsCal = needsCalendar(cfg.conditions);
        ...
        <CardContent className="pt-0 flex flex-col gap-3">
          ...
          <div className="flex flex-col gap-2">
            <div className="flex items-center gap-2">
              ...
              <Button variant="ghost" size="sm" onClick={() => setConditions(prev => [...prev, newStudyCondition("sigma")])}
...
function BiggestMoves({ tickers, sigmaWindow, sigmaBasis, dates }: {
  ...
  const moveSort = useTableSort<any>("", "desc", "desc", "studyfamily-moves");
