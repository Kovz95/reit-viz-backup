...
function MacroPane({
  ...
  useEffect(() => {
    ...
    // Full indicator suite (MAs, bands, registry, overlays) — shared with the
    // Levels page; see lib/bandIndicators. Series register into the ref so the
    // next render disposes them.
    renderBandIndicators(chart, primaryData as unknown as { time: string; value: number }[], activeIndicators, (s) => indicatorSeriesRef.current.push(s));
