...
function te(s, n) {
    ...
    switch (s) {
        ...
        case "stoch_kd":
            ...
            break;
        case "ma_slope":
            a.maType = a.maType ?? "EMA", a.period = a.period ?? 50, a.slopeLookback = a.slopeLookback ?? 5;
            break;
        case "price_vs_ma":
            a.maType = a.maType ?? "SMA", a.period = a.period ?? 50;
            break;
        case "ma_spread":
            a.fastMaType = a.fastMaType ?? "EMA", a.fastPeriod = a.fastPeriod ?? 20, a.slowMaType = a.slowMaType ?? "EMA", a.slowPeriod = a.slowPeriod ?? 50;
            break;
        case "harsi_rsi":
        case "harsi_ha_close":
        case "harsi_stoch_k":
...
function Cs() {
    ...
    g.useEffect(() => {
        Lt().then(t => {
            a(t), t.length > 0 && (S(m => m || t[0].ticker), z(m => m || (t[1]?.ticker ?? t[0].ticker)))
        })
    }, []);
    ...
    const [P, ne] = g.useState(2), [Y, Ee] = g.useState(5), [V, He] = g.useState(21), [Q, ze] = g.useState(30), [Z, De] = g.useState(1.5), [pe, We] = g.useState(40), [ae, qe] = g.useState(!1), [le, Ue] = g.useState(70), [M, Se] = g.useState(!1), {
        frequency: Ce,
        setFrequency: Me,
        frequencyUI: lt
        ...
        fetched: 0,
        fetchTotal: 0
    }), [Ae, be] = Pe("range-input-selection", zt), [Ke, Ye] = Pe("range:result", null), [it, _e] = g.useState(null), [Ve, ue] = g.useState(null), [ct, dt] = g.useState(25), [mt, ht] = g.useState(25), [Je, ke] = g.useState(null), Te = g.useCallback(() => ({
        mode: o,
        selectedTicker: l,
        pairTickerA: x,
        pairTickerB: b,
        ...
        if (t) {
            if ((t.mode === "single" || t.mode === "pool" || t.mode === "pair" || t.mode === "pairCombo" || t.mode === "basket") && i(t.mode), t.pairCombo && d.hydrate(t.pairCombo), typeof t.pairTickerA == "string" && S(t.pairTickerA), typeof t.pairTickerB == "string" && z(t.pairTickerB), Array.isArray(t.basketTickers) && f(t.basketTickers.filter(m => typeof m == "string")), (t.basketMode === "stocks" || t.basketMode === "combined") && _(t.basketMode), t.selectedTicker && y(t.selectedTicker), Array.isArray(t.indicators) && t.indicators.length > 0) {
            ...
            if (Array.isArray(t.selectedIds) && se(new Set(t.selectedIds)), (t.comboSize === 2 || t.comboSize === 3) && ne(t.comboSize), typeof t.bins == "number" && Ee(t.bins), typeof t.horizonDays == "number" && He(t.horizonDays), typeof t.minHits == "number" && ze(t.minHits), typeof t.minLiftPct == "number" && De(t.minLiftPct), typeof t.maxPoolTickers == "number" && We(t.maxPoolTickers), typeof t.walkForwardEnabled == "boolean" && qe(t.walkForwardEnabled), typeof t.trainPct == "number" && Ue(t.trainPct), t.frequency === "daily" || t.frequency === "weekly" || t.frequency === "weekly_on_daily" ? Me(t.frequency) : t.timeframe === "weekly" && Me("weekly"), t.inputSelection && typeof t.inputSelection == "object") {
    ...
    const jt = () => (oe.current || (oe.current = new os), oe.current),
        ...
        wt = async () => {
            ...
            const N = Re({
                ...
                B = re === "weekly" ? 52 : 252;
            if (N.adjCloses.length < B) throw new Error(`Insufficient history for ${l} (need ≥${B} ${re} bars).`);
            E({
                current: 0,
                total: 0,
                stage: "compute",
            ...
            const t = await Qe(),
                m = await Ze(x, b, t);
            if (!m || m.indices.length < 252) throw new Error(`Insufficient pair-ratio history for ${x}/${b} (need ≥252 daily bars).`);
            ...
            if (await Promise.all(D), p.length === 0) throw new Error("No pair combos had sufficient ratio history (need ≥252 daily bars).");
            E({
                current: 0,
                total: 0,
                stage: "search",
            ...
            if (h.length === 0) throw new Error("No tickers available — adjust your universe/classification filter.");
            ...
            if (await Promise.all(D), p.length === 0) throw new Error("No tickers had sufficient Yahoo history.");
            E({
                current: 0,
                total: 0,
                stage: "search",
            ...
            if (R === "combined") {
                ...
                if (!u || u.closes.length < 252) throw new Error("Insufficient history for basket combined series (need ≥252 bars).");
                E({
                    current: 0,
                    total: 0,
                    stage: "fetch",
            ...
            if (await Promise.all(A), m.length === 0) throw new Error("No basket tickers had sufficient Yahoo history.");
            E({
                current: 0,
                total: 0,
                stage: "search",
            ...
            const N = jt(),
                ...
                A = q => {
                    ...
                    if (!(!w || w.id !== B))
                        ...
                        if (N.removeEventListener("message", A), Ye(w.result), w.result.longsByTicker || w.result.shortsByTicker) {
                        ...
                        p()
                    } else w.type === "error" && (N.removeEventListener("message", A), j(new Error(w.error)))
                };
            ...
            try {
                o === "single" ? await wt() : o === "pair" ? await vt() : o === "pairCombo" ? await Nt() : o === "basket" ? await Ct() : await St()
            } catch (t) {
    ...
    return e.jsxs("div", {
        ...
        children: [e.jsxs("div", {
            ...
            children: [e.jsx("button", {
                ...
                children: o === "single" ? "Bands learned from one stock's history" : o === "pair" ? `Bands learned from ratio ${x||"A"}/${b||"B"} (daily only)` : o === "pairCombo" ? `Bands learned across ${d.pairs.length} pair${d.pairs.length!==1?"s":""} (daily only)` : o === "basket" ? `Basket of ${F.length} ticker${F.length!==1?"s":""}` : `Bands learned across ${Math.min(pe,h.length)} tickers${c.hasActiveFilters?" (filtered)":""}`
            })]
            ...
            children: [o !== "pair" && o !== "pairCombo" && lt, o === "single" ? e.jsxs("div", {
                ...
                children: [e.jsx("label", {
                    ...
                    children: ["of ", h.length, c.hasActiveFilters ? ` filtered (${r.length} total)` : " in universe"]
                })]
            }), e.jsxs("div", {
                className: "flex flex-col gap-0.5",
                children: [e.jsx("label", {
                    ...
                    children: o === "single" ? e.jsxs(e.Fragment, {
                        children: [e.jsxs("label", {
                            ...
                            children: [e.jsx("input", {
                                ...
                                max: 95,
                                value: le,
                                onChange: t => Ue(Math.min(95, Math.max(30, parseInt(t.target.value, 10) || 70))),
                                disabled: M,
                ...
                variant: "default",
                onClick: Mt,
                disabled: H.length < P || o === "single" && !l || o === "pair" && (!x || !b || x === b) || o === "basket" && F.length === 0 || o === "pairCombo" && d.pairs.length === 0,
                className: "h-8",
                children: [e.jsx(Gt, {
                    className: "w-3.5 h-3.5 mr-1"
                }), "Run Search"]
                ...
                children: [Fe, " combo", Fe === 1 ? "" : "s", " × ", Math.pow(Y, P), " buckets each"]
            }), M && e.jsxs("div", {
            ...
            setLongLimit: dt,
            setShortLimit: ht,
            tickerLabel: o === "single" ? l : o === "pair" ? `${x}/${b}` : o === "pairCombo" ? `pair-combo-${d.pairs.length}` : o === "basket" ? `basket-${F.length}` : `pool-${h.length}`
        })]
    })
}
...
function ls({
    ...
    })), x = r.kind, S = x === "ma_slope" || x === "price_vs_ma", b = x === "ma_spread", z = x === "stoch_k" || x === "stoch_d" || x === "stoch_kd", F = x.startsWith("harsi_");
    return e.jsx("div", {
...
function ds({
    ...
    const l = nt.find(S => S.days === r)?.label ?? `${r}d`,
        y = !!s.walkForward,
        x = g.useCallback(() => {
            const S = ["Side", "Band", "Hits", "Win%", "MeanRet", "Med", "Std", "Lift", "tStat", "LastHit", "CurrentlyIn"];
            y && S.push("OOS_Hits", "OOS_Mean", "OOS_Lift", "OOS_Win%"), n === "pool" && S.push("Tickers_Now");
            const b = [S],
                z = (v, W, U) => {
                    const se = v.parts.map(ne => ne.display).join(" & "),
                        P = [W, se, v.hits, (v.winRate * 100).toFixed(2), (v.meanReturn * 100).toFixed(3), (v.medianReturn * 100).toFixed(3), (v.stdReturn * 100).toFixed(3), (v.lift * 100).toFixed(3), v.tStat.toFixed(3), v.lastDate ?? "", v.currentlyIn ? "Y" : ""];
                    if (y && P.push(v.oosHits ?? 0, v.oosMean !== void 0 ? (v.oosMean * 100).toFixed(3) : "", v.oosLift !== void 0 ? (v.oosLift * 100).toFixed(3) : "", v.oosWinRate !== void 0 ? (v.oosWinRate * 100).toFixed(2) : ""), n === "pool") {
...
`),
            ...
            _.href = R, _.download = `range-opt-${h||"results"}-${l}-${Ne}.csv`, document.body.appendChild(_), _.click(), setTimeout(() => {
    ...
    return e.jsxs("div", {
        ...
        children: [e.jsxs("div", {
            ...
            children: [e.jsxs("span", {
                ...
                children: ["μ ", we(s.baselineMean), " · σ ", we(s.baselineStd), " · n ", s.baselineN.toLocaleString()]
            }), y && s.oosBaselineMean !== void 0 && s.oosBaselineN !== void 0 && e.jsxs("span", {
                className: "text-[11px] font-mono text-cyan-400",
                children: ["OOS: μ ", we(s.oosBaselineMean), " · n ", s.oosBaselineN.toLocaleString()]
            }), e.jsxs("span", {
                className: "text-[10px] font-mono text-muted-foreground",
                children: [s.totalBuckets.toLocaleString(), " buckets · ", n === "pool" ? "pool / cross-sectional" : "single ticker", y && " · walk-forward"]
            }), e.jsxs(he, {
                size: "sm",
                variant: "outline",
                onClick: x,
    ...
    return a.length === 0 ? e.jsxs("div", {
        ...
        children: [e.jsxs("div", {
            ...
            children: [e.jsx("span", {
                ...
                children: ["— tickers currently in qualifying bands (", r.length, " long-leaning, ", o.length, " short-leaning)"]
            })]
                    ...
                    children: h.map((l, y) => {
                        ...
                        return e.jsxs("tr", {
                            ...
                            children: [e.jsx("td", {
                                ...
                                children: e.jsx("div", {
                                    className: "flex flex-col gap-0.5",
                                    children: l.parts.map((b, z) => e.jsx("span", {
                                ...
                                children: [e.jsx("td", {
                                    ...
                                    children: l.oosMean !== void 0 ? ge(l.oosMean) : "—"
                                }), e.jsx("td", {
                                    ...
                                    title: (l.oosLift ?? 0) > 0 && l.lift > 0 || (l.oosLift ?? 0) < 0 && l.lift < 0 ? "Confirmed in OOS" : "Sign flipped in OOS — weak edge",
                                    children: l.oosLift !== void 0 ? ge(l.oosLift) : "—"
                                }), e.jsx("td", {
                                    className: "px-2 py-1 text-right text-muted-foreground",
                                    children: l.oosWinRate !== void 0 ? `${(l.oosWinRate*100).toFixed(0)}%` : "—"
                                })]
                            }), e.jsx("td", {
                                className: "px-2 py-1 text-right text-muted-foreground",
                                children: l.lastDate ?? "—"
                                ...
                                children: S ? x.length > 0 ? e.jsxs("span", {
                                    ...
                                    title: x.join(", "),
                                    children: [x.slice(0, 3).join(", "), x.length > 3 ? ` +${x.length-3}` : ""]
                                }) : e.jsx("span", {
