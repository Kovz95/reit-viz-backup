...
async function $e(n) {
    ...
    try {
        const c = await Ce(n);
        c.dates.length && (s = c.dates, l = c.closes, u = c.highs, o = c.lows, i = c.opens, a = c.volumes, g = a.some(b => Number.isFinite(b) && b > 0))
    } catch {
        const c = await Ze(n);
        s = c.map(b => b.time), l = c.map(b => b.close), u = l.slice(), o = l.slice(), i = l.slice(), a = new Array(l.length).fill(0)
    }
    return {
        times: s,
        closes: l,
...
async function Ye(n) {
    ...
    try {
        ...
        if (N.length + C.length === 0) return s.status = "skipped", s.errorMsg = "no valid features", s;
        const G = nt(N, {
                closes: o,
                highs: i,
                lows: a,
        ...
        const r = [...N, ...C],
            y = o.length - 1,
            Y = r.map(d => $[d][y]);
        if (Y.some(d => !Number.isFinite(d))) return s.status = "skipped", s.errorMsg = "today z has NaN", s;
        ...
        return s.status = "ok", s.matchN = I.matches.length, s.baseN = k.length, A && (s.median1M = A.median, s.mean1M = A.mean, s.p25_1M = A.p25, s.p75_1M = A.p75, s.hitLong1M = A.hitLong, s.hitShort1M = A.hitShort, s.baseLong1M = A.baseLong, s.baseShort1M = A.baseShort), E && (s.median3M = E.median, s.mean3M = E.mean, s.p25_3M = E.p25, s.p75_3M = E.p75, s.hitLong3M = E.hitLong, s.hitShort3M = E.hitShort, s.baseLong3M = E.baseLong, s.baseShort3M = E.baseShort), T && (s.median6M = T.median, s.mean6M = T.mean, s.hitLong6M = T.hitLong, s.hitShort6M = T.hitShort, s.baseLong6M = T.baseLong, s.baseShort6M = T.baseShort), D && (s.median1Y = D.median, s.mean1Y = D.mean, s.hitLong1Y = D.hitLong, s.hitShort1Y = D.hitShort, s.baseLong1Y = D.baseLong, s.baseShort1Y = D.baseShort), s
    } catch (l) {
...
async function mt(n) {
    ...
    try {
        ...
        if (l.closes.length < 252) return s.status = "skipped", s.errorMsg = `only ${l.closes.length} bars`, s;
        ...
        if (s.consensus_totalPresets = o.length, s.consensus_validPresets = i.length, s.consensus_perPreset = o.map(r => ({
                preset: r.preset,
                n: r.r.matchN ?? 0,
        ...
        s.median1M = a(b, c), s.median3M = a(w, c), s.median6M = a(z, c), s.median1Y = a(h, c), s.consensus_sd1M = g(b), s.consensus_sd3M = g(w), s.consensus_sd6M = g(z), s.consensus_sd1Y = g(h), s.mean1M = a(i.map(r => r.r.mean1M ?? NaN), c), s.mean3M = a(i.map(r => r.r.mean3M ?? NaN), c), s.mean6M = a(i.map(r => r.r.mean6M ?? NaN), c), s.mean1Y = a(i.map(r => r.r.mean1Y ?? NaN), c), s.hitLong3M = a(i.map(r => r.r.hitLong3M ?? NaN), c), s.hitShort3M = a(i.map(r => r.r.hitShort3M ?? NaN), c), s.baseLong3M = a(i.map(r => r.r.baseLong3M ?? NaN), c), s.baseShort3M = a(i.map(r => r.r.baseShort3M ?? NaN), c), s.hitLong1M = a(i.map(r => r.r.hitLong1M ?? NaN), c), s.hitShort1M = a(i.map(r => r.r.hitShort1M ?? NaN), c), s.baseLong1M = a(i.map(r => r.r.baseLong1M ?? NaN), c), s.baseShort1M = a(i.map(r => r.r.baseShort1M ?? NaN), c), s.hitLong6M = a(i.map(r => r.r.hitLong6M ?? NaN), c), s.hitShort6M = a(i.map(r => r.r.hitShort6M ?? NaN), c), s.baseLong6M = a(i.map(r => r.r.baseLong6M ?? NaN), c), s.baseShort6M = a(i.map(r => r.r.baseShort6M ?? NaN), c), s.hitLong1Y = a(i.map(r => r.r.hitLong1Y ?? NaN), c), s.hitShort1Y = a(i.map(r => r.r.hitShort1Y ?? NaN), c), s.baseLong1Y = a(i.map(r => r.r.baseLong1Y ?? NaN), c), s.baseShort1Y = a(i.map(r => r.r.baseShort1Y ?? NaN), c), s.matchN = i.reduce((r, y) => r + (y.r.matchN ?? 0), 0), s.baseN = i.reduce((r, y) => r + (y.r.baseN ?? 0), 0);
        ...
        s.consensus_long3M = N, s.consensus_short3M = C, s.consensus_agreement = F, s.consensus_direction = G;
        const P = Number.isFinite(s.consensus_sd3M) && s.consensus_sd3M > 0 ? Math.abs(s.median3M) / s.consensus_sd3M : NaN;
        s.consensus_snr = P;
        let $ = "Mixed — no clear direction";
        if (G !== "mixed") {
            const r = G === "long" ? "long" : "short";
            F >= .8 && (P >= 1 || !Number.isFinite(P)) ? $ = `Strong ${r}` : F >= .66 ? $ = `Moderate ${r}` : F >= .55 ? $ = `Weak ${r}` : $ = `Mixed — ${r} leaning`
        }
...
function ft() {
    ...
    const [o, i] = x.useState("all"), [a, g] = x.useState("sector"), [c, b] = x.useState(""), [w, z] = x.useState(""), {
        metas: h,
        loading: N,
        error: C
    } = Qe(), [F, G] = x.useState("sector"), [P, $] = x.useState(""), [r, y] = x.useState("knn"), [Y, H] = x.useState(60), [k, O] = x.useState(0), [I, Z] = x.useState(5), [A, E] = x.useState(20), [T, D] = x.useState(252), [d, B] = x.useState(() => new Set(et)), [p, Q] = x.useState("3M"), [f, se] = x.useState(!1), [ee, me] = x.useState(() => new Set(Object.keys(X).filter(e => e !== "Classic (6)"))), [re, ue] = x.useState(!1), [ne, be] = x.useState({
        done: 0,
        total: 0
    }), [V, he] = x.useState([]), [Ne, pe] = x.useState(null), ie = x.useRef(!1), [ce, Ae] = x.useState("composite"), [oe, fe] = x.useState("desc"), te = x.useMemo(() => {
        const e = new Set;
        for (const m of l) {
            const v = m[a];
            v && e.add(String(v))
    ...
    const U = x.useMemo(() => {
            if (o === "global") return h.length === 0 ? [] : P ? h.filter(e => String(e[F] ?? "") === P).map(e => e.ticker) : h.map(e => e.ticker);
            if (l.length === 0) return [];
            if (o === "all") return l.map(e => e.ticker);
            if (o === "classification" && c) return l.filter(e => String(e[a] ?? "") === c).map(e => e.ticker);
            if (o === "basket" && w) {
        ...
        Ee = async () => {
            ...
            const e = J.filter(M => M.status === "ok"),
                ...
                v = M => {
                    ...
                    switch (ce) {
                        ...
                        case "dispersion":
                            return (p === "1M" ? M.consensus_sd1M : p === "3M" ? M.consensus_sd3M : p === "6M" ? M.consensus_sd6M : M.consensus_sd1Y) ?? NaN;
                        case "snr":
            ...
            return e.sort((M, L) => {
                const R = v(M),
                    j = v(L);
                if (typeof R == "string" && typeof j == "string") return oe === "asc" ? R.localeCompare(j) : j.localeCompare(R);
        ...
        }, [J, ce, oe, p]), Me = x.useMemo(() => [...J].filter(e => e.status === "ok" && Number.isFinite(e.composite ?? NaN)).sort((e, m) => (m.composite ?? -1 / 0) - (e.composite ?? -1 / 0)).slice(0, 10), [J]), je = x.useMemo(() => [...J].filter(e => e.status === "ok" && Number.isFinite(e.composite ?? NaN)).sort((e, m) => (e.composite ?? 1 / 0) - (m.composite ?? 1 / 0)).slice(0, 10), [J]), Oe = e => {
            ce === e ? fe(oe === "asc" ? "desc" : "asc") : (Ae(e), fe(e === "ticker" ? "asc" : "desc"))
        }, le = e => Number.isFinite(e ?? NaN) ? `${e>=0?"+":""}${e.toFixed(1)}%` : "—", Se = e => Number.isFinite(e ?? NaN) ? `${e>=0?"+":""}${e.toFixed(0)}pp` : "—", ke = e => Number.isFinite(e ?? NaN) ? e.toFixed(2) : "—", Ie = e => {
        ...
        }, ze = o === "all" ? `All (${l.length})` : o === "global" ? N ? "Global (loading…)" : `Global · ${P||"all"} (${U.length})` : o === "classification" ? `${a}=${c} (${U.length})` : o === "basket" ? (() => {
            const e = n.find(m => m.id === w);
    ...
    return t.jsxs("div", {
        ...
        children: [t.jsxs("div", {
            ...
            children: [t.jsx(He, {
                ...
                children: [V.filter(e => e.status === "ok").length, " ok ·", " ", V.filter(e => e.status === "skipped").length, " skipped ·", " ", V.filter(e => e.status === "error").length, " error"]
            }), t.jsxs("span", {
            ...
            children: [t.jsx("span", {
                ...
                children: ["all", "classification", "basket", "global"].map(e => t.jsx("button", {
                    onClick: () => i(e),
                    ...
                    title: e === "global" ? "FactSet/RBICS GLOBAL universe (~9,000 tickers, Yahoo-priced)" : void 0,
                    children: e
                }, e))
                ...
                children: [t.jsx("label", {
                    ...
                    className: "text-[11px] font-mono px-1.5 py-0.5 bg-background border border-border rounded text-foreground",
                    children: [20, 30, 40, 60, 90, 120, 180, 252].map(e => t.jsxs("option", {
                        value: e,
                        children: [e, "b"]
                    }, e))
                })]
            ...
            children: [t.jsx("span", {
                ...
                children: [ee.size, " preset", ee.size === 1 ? "" : "s", " selected · per-ticker runs ", ee.size, "× algorithm"]
            })]
            ...
            children: [V.length === 0 && !re && t.jsxs("div", {
                ...
                children: ["Set up the universe and parameters above, then click ", t.jsx("span", {
                    className: "text-amber-300",
                    children: "Run"
                }), " to screen.", t.jsx("br", {}), "First run fetches OHLCV per ticker (cached on later runs)."]
                ...
                children: [t.jsx("thead", {
                    ...
                    children: t.jsxs("tr", {
                        ...
                        children: [_("ticker", "Ticker", "left"), _("composite", `Composite ${p}`), _("median", `Median ${p}`), !f && _("mean", `Mean ${p}`), !f && (p === "1M" || p === "3M") && _("p25", `p25 ${p}`), !f && (p === "1M" || p === "3M") && _("p75", `p75 ${p}`), _("hitLong", `L hit ${p}`), _("hitLongEdge", "L edge"), _("hitShort", `S hit ${p}`), _("hitShortEdge", "S edge"), f && _("agreement", "Agreement"), f && _("dispersion", `±sd ${p}`), f && _("snr", "SNR"), f && _("validPresets", "Presets"), !f && _("n", "matches"), f && t.jsx("th", {
                    ...
                    children: Re.map(e => {
                        const m = ae(e, p),
                            v = j => Number.isFinite(j ?? NaN) ? j >= 0 ? "text-green-400" : "text-red-400" : "text-muted-foreground/50",
                            M = (j, S) => Number.isFinite(j) ? S ? j >= 0 ? "text-green-400/80" : "text-red-400/80" : j >= 0 ? "text-red-400/80" : "text-green-400/80" : "text-muted-foreground/50",
                            L = (m.hL ?? NaN) - (m.bL ?? NaN),
                            R = (m.hS ?? NaN) - (m.bS ?? NaN);
                        return t.jsxs("tr", {
                            ...
                            children: [t.jsx("td", {
                                ...
                                children: Number.isFinite(m.hL ?? NaN) ? `${m.hL.toFixed(0)}%` : "—"
                            }), t.jsx("td", {
                                className: `text-right py-0.5 px-2 ${M(L,!0)}`,
                                children: Se(L)
                            }), t.jsx("td", {
                                className: "text-right py-0.5 px-2 text-red-400",
                                children: Number.isFinite(m.hS ?? NaN) ? `${m.hS.toFixed(0)}%` : "—"
                            }), t.jsx("td", {
                                ...
                                children: [t.jsx("td", {
                                    ...
                                    children: Number.isFinite(e.consensus_agreement ?? NaN) ? `${Math.round(e.consensus_agreement*100)}%` : "—"
                                }), t.jsx("td", {
                                    className: "text-right py-0.5 px-2 text-muted-foreground",
                                    children: (() => {
                                        const j = p === "1M" ? e.consensus_sd1M : p === "3M" ? e.consensus_sd3M : p === "6M" ? e.consensus_sd6M : e.consensus_sd1Y;
                                        return Number.isFinite(j ?? NaN) ? `±${j.toFixed(1)}pp` : "—"
                                    })()
                                }), t.jsx("td", {
                                    className: "text-right py-0.5 px-2 text-muted-foreground",
                                    ...
                                    className: "text-right py-0.5 px-2 text-muted-foreground",
                                    children: [e.consensus_validPresets ?? "—", e.consensus_totalPresets ? `/${e.consensus_totalPresets}` : ""]
                                })]
                            }), !f && t.jsx("td", {
                                className: "text-right py-0.5 px-2 text-muted-foreground",
                                children: e.matchN ?? "—"
                                ...
                                children: (() => {
                                    ...
                                    const K = j.startsWith("Strong") && S === "long" ? "text-green-300 font-semibold" : j.startsWith("Strong") && S === "short" ? "text-red-300 font-semibold" : j.startsWith("Moderate") && S === "long" ? "text-green-400/90" : j.startsWith("Moderate") && S === "short" ? "text-red-400/90" : j.startsWith("Weak") && S === "long" ? "text-green-400/70" : j.startsWith("Weak") && S === "short" ? "text-red-400/70" : "text-muted-foreground";
                                    return t.jsx("span", {
                                        className: K,
                                        children: j
                                    })
                                ...
                                children: [e.status === "running" && t.jsx("span", {
                                    className: "text-amber-300",
                                    children: "running…"
                                }), e.status === "pending" && t.jsx("span", {
                                    className: "text-muted-foreground/60",
                                    children: "queued"
                                }), e.status === "ok" && t.jsx("span", {
                                    className: "text-green-400/80",
                                    children: "ok"
                                }), e.status === "skipped" && t.jsx("span", {
...
function _e({
    ...
    const i = s === "long" ? "text-green-400" : "text-red-400",
        a = g => Number.isFinite(g ?? NaN) ? g >= 0 ? "text-green-400" : "text-red-400" : "text-muted-foreground/50";
    return t.jsxs("div", {
                ...
                children: l.map((g, c) => {
                    ...
                    return t.jsxs("tr", {
                        ...
                        children: [t.jsx("td", {
                            ...
                            children: Number.isFinite(g.composite ?? NaN) ? g.composite.toFixed(2) : "—"
                        }), t.jsx("td", {
                            className: `text-right py-0.5 pr-2 ${a(b.med)}`,
                            children: Number.isFinite(b.med ?? NaN) ? `${b.med>=0?"+":""}${b.med.toFixed(1)}%` : "—"
                        }), t.jsx("td", {
                            className: "text-right py-0.5 pr-2 text-green-400",
                            children: Number.isFinite(b.hL ?? NaN) ? `${b.hL.toFixed(0)}` : "—"
                        }), t.jsx("td", {
                            className: "text-right py-0.5 text-red-400",
                            children: Number.isFinite(b.hS ?? NaN) ? `${b.hS.toFixed(0)}` : "—"
                        })]
                    }, g.ticker)
                })
            })]
