                ...
                try {
                    ...
                    for (let c = $[0]; c >= Math.max(0, $[0] - 10); c--)
                        if (c < l.length && y.has(l[c])) {
                            e = y.get(l[c]);
                            break
                        } o.divYield = V, o.treasuryRate = e, o.spread = P, o.mean = D, o.std = j, o.zScore = I, o.histPctile = W, o.spreadSeries = T, o.sparkValues = i.slice(-200)
                } catch {}
                return o
            }));
        v.push(...k)
...
function ke({
    ...
    return m.useEffect(() => {
        ...
        n.width = g * v, n.height = p * v, n.style.width = `${g}px`, n.style.height = `${p}px`, a.scale(v, v);
        let F = [...s];
        ...
        if (a.clearRect(0, 0, g, p), r !== null && x !== null) {
            const i = N(r + x),
                D = N(r - x);
            a.fillStyle = "rgba(14, 165, 233, 0.07)", a.fillRect(u, i, o, D - i), a.beginPath(), a.strokeStyle = "rgba(14, 165, 233, 0.35)", a.lineWidth = .8, a.setLineDash([3, 3]);
            const j = N(r);
...
function O(s) {
    return s === null ? "" : s >= 2 ? "text-green-400 font-semibold" : s >= 1 ? "text-green-400" : s >= .5 ? "text-green-300/80" : s <= -2 ? "text-red-400 font-semibold" : s <= -1 ? "text-red-400" : s <= -.5 ? "text-red-300/80" : "text-muted-foreground"
}

function Ne(s) {
    return s === null ? "" : s >= 2 ? "bg-green-500/10" : s >= 1 ? "bg-green-500/5" : s <= -2 ? "bg-red-500/10" : s <= -1 ? "bg-red-500/5" : ""
}

function K(s) {
    return s === null ? "" : s > 80 ? "text-green-400" : s < 20 ? "text-red-400" : "text-muted-foreground"
}
...
function Re() {
    const {
        universeTickers: s
    } = X(), [r, x] = m.useState("DGS10"), [l, g] = m.useState(1260), [p, y] = m.useState("spread"), [n, a] = m.useState("desc"), [v, F] = m.useState(""), [d, b] = m.useState(J), [k, u] = m.useState(new Set), [o, S] = m.useState(null), N = q.find(e => e.id === r)?.label ?? r, T = m.useCallback(() => ({
        treasuryId: r,
        lookback: l,
        sortCol: p,
        sortDir: n,
        ...
        e.treasuryId !== void 0 && x(e.treasuryId), e.lookback !== void 0 && g(e.lookback), e.sortCol !== void 0 && y(e.sortCol), e.sortDir !== void 0 && a(e.sortDir), e.search !== void 0 && F(e.search), e.classFilters !== void 0 && b(w(e.classFilters)), e.manualTickers !== void 0 && u(new Set(e.manualTickers)), e.selectedTicker !== void 0 && S(e.selectedTicker)
    }, []);
    ...
    const {
        ...
        return s && (e = e.filter(c => s.has(c.ticker))), e = se(e, d, v, k), e.sort((c, C) => {
            ...
            if (p === "ticker") return n === "asc" ? c.ticker.localeCompare(C.ticker) : C.ticker.localeCompare(c.ticker);
            const R = z(c),
                f = z(C);
        ...
        const e = ["Rank", "Ticker", "Name", "Subindustry", "Div Yield %", `${N} %`, "Spread %", "Mean %", "Z-Score", "Hist %ile"],
            c = j.map((h, M) => [M + 1, h.ticker, `"${h.name}"`, `"${h.subindustry}"`, h.divYield?.toFixed(2) ?? "", h.treasuryRate?.toFixed(2) ?? "", h.spread?.toFixed(2) ?? "", h.mean?.toFixed(2) ?? "", h.zScore?.toFixed(2) ?? "", h.histPctile?.toFixed(0) ?? ""].join(",")),
            C = [e.join(","), ...c].join(`
`),
            z = new Blob([C], {
                type: "text/csv"
                    ...
                    children: j.map((e, c) => {
                        ...
                        return t.jsxs("tr", {
                            ...
                            children: [t.jsx("td", {
                                ...
                                children: e.spread !== null ? `${e.spread>0?"+":""}${e.spread.toFixed(2)}%` : "—"
                            }), t.jsxs("td", {
                                className: "px-2 py-1 text-right font-mono tabular-nums text-muted-foreground",
