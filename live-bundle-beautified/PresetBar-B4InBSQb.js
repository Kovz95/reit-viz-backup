...
const z = new Set(["selectedTicker", "pairTickerA", "pairTickerB", "results", "gridResults", "expandedTicker", "expandedGridTicker", "sortBy", "runSort", "gridLongSort", "gridShortSort", "evalResult", "evalTriggerKey", "evalFilterKeys"]);
...
function U({
    ...
    const n = i.find(e => e.id === o),
        K = t.useCallback(() => {
            n && (l(n.inputs), D(n.name), y.current && clearTimeout(y.current), y.current = setTimeout(() => D(null), 2e3))
        }, [n, l]),
        j = t.useCallback(() => {
            const e = f.trim();
            if (!e) return;
