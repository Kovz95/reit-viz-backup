...
const _ = {
  Valuation: ["P/FFO FY2", "P/FFO LTM", "P/AFFO FY2", "P/AFFO LTM", "P/E FY2", "P/E LTM",
    "P/S FY2", "P/S LTM", "EV/EBITDA FY2", "EV/EBITDA LTM", "Implied Cap Rate"
  ],
  Yields: ["FFO Yield FY2", "FFO Yield LTM", "AFFO Yield FY2", "AFFO Yield LTM",
    "Dividend Yield"],
...
function fe({
  ...
  return p.useEffect(() => {
    ...
    d.width = b * N, d.height = v * N, d.style.width = `${b}px`, d.style.height = `${v}px`, s
      .scale(N, N);
    let L = [...a];
    ...
    if (s.clearRect(0, 0, b, v), i !== null && h !== null) {
      const x = g(i + h),
        S = g(i - h);
      s.fillStyle = "rgba(14, 165, 233, 0.07)", s.fillRect(f, x, C, S - x), s.beginPath(), s
        .strokeStyle = "rgba(14, 165, 233, 0.35)", s.lineWidth = .8, s.setLineDash([3, 3]);
      const T = g(i);
      s.moveTo(f, T), s.lineTo(f + C, T), s.stroke(), s.setLineDash([]), s.beginPath(), s
        .strokeStyle = "rgba(14, 165, 233, 0.15)", s.lineWidth = .5, s.setLineDash([2, 2]);
      const M = g(i + h);
...
function q(a) {
  return a === null ? "" : a <= -2 ? "text-green-400 font-semibold" : a <= -1 ? "text-green-400" :
    a <= -.5 ? "text-green-300/80" : a >= 2 ? "text-red-400 font-semibold" : a >= 1 ?
    "text-red-400" : a >= .5 ? "text-red-300/80" : "text-muted-foreground"
}

function ge(a) {
  return a === null ? "" : a <= -2 ? "bg-green-500/10" : a <= -1 ? "bg-green-500/5" : a >= 2 ?
    "bg-red-500/10" : a >= 1 ? "bg-red-500/5" : ""
}

function ye() {
  const {
    universeTickers: a
  } = Q(), [i, h] = p.useState("P/FFO FY2"), [u, b] = p.useState(1260), [v, z] = p.useState(
      "zScore"), [d, s] = p.useState("asc"), [N, L] = p.useState(""), [k, $] = p.useState(X), [Y,
    f] = p.useState(new Set), [C, E] = p.useState(new Set), [g, O] = p.useState("none"), I = p
    .useCallback(() => ({
      metric: i,
      lookback: u,
      sortCol: v,
      ...
      e.metric !== void 0 && h(e.metric), e.lookback !== void 0 && b(e.lookback), e.sortCol !==
        void 0 && z(e.sortCol), e.sortDir !== void 0 && s(e.sortDir), e.classFilters !== void 0 &&
        $(ee(e.classFilters)), e.manualTickers !== void 0 && f(new Set(e.manualTickers)), e
        .groupByLevel !== void 0 ? O(e.groupByLevel) : e.groupBySubind !== void 0 && O(e
          .groupBySubind ? "subindustry" : "none")
    }, []);
  te("valuation", I, x);
  const {
    data: S = [],
    ...
    const {
      mean: n,
      std: l
    } = pe(e.values), o = e.current, c = o !== null && l > 0 ? (o - n) / l : null, m = o !==
      null && e.values.length > 1 ? e.values.filter(F => F < o).length / (e.values.length -
      1) * 100 : o !== null && e.values.length === 1 ? 50 : null, r = o !== null && n !== 0 ?
      (o - n) / n * 100 : null;
    return {
      ticker: e.ticker,
      name: e.name,
    ...
    return a && (e = e.filter(n => a.has(n.ticker))), e = se(e, k, N, Y), e.sort((n, l) => {
      ...
      if (v === "ticker") return d === "asc" ? n.ticker.localeCompare(l.ticker) : l.ticker
        .localeCompare(n.ticker);
      const c = o(n),
        m = o(l);
    ...
    const e = ["Rank", "Ticker", "Name", "Subindustry", `Current ${i}`, "5Y Mean", "5Y Std Dev",
        "Z-Score", "Pctile", "Premium/Disc %"
      ],
      n = y.map((r, F) => [F + 1, r.ticker, `"${r.name}"`, `"${r.subindustry}"`, r.current
        ?.toFixed(2) ?? "", r.mean5Y?.toFixed(2) ?? "", r.std5Y?.toFixed(2) ?? "", r.zScore
        ?.toFixed(2) ?? "", r.histPctile?.toFixed(1) ?? "", r.premium?.toFixed(1) ?? ""
      ].join(",")),
      l = [e.join(","), ...n].join(`
`),
      o = new Blob([l], {
    ...
    m.href = c, m.download = `valuation_${i.replace(/[^a-zA-Z0-9]/g,"_")}.csv`, m.click(), URL
      .revokeObjectURL(c)
  }, A = p.useMemo(() => {
      ...
      children: T ? t.jsx("div", {
        ...
        children: [t.jsx("thead", {
          ...
          children: g !== "none" && Z ? Z.map(([e, n]) => {
            const l = C.has(e),
              o = n.filter(c => c.zScore !== null).length > 0 ? n.filter(
                c => c.zScore !== null).reduce((c, m) => c + m.zScore, 0) /
              n.filter(c => c.zScore !== null).length : null;
