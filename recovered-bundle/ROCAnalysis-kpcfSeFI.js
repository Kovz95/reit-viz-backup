...
function ne(t, s) {
  ...
  let h = 0,
    ...
    m = 0,
    x = 0;
  for (let f = 0; f < a; f++) h += l[f], c += i[f], u += l[f] * i[f], m += l[f] * l[f], x += i[f] *
    i[f];
  const r = a * u - h * c,
...
function oe(t, s) {
  ...
  let o = 0,
    ...
    h = 0,
    c = 0;
  for (let x = 0; x < a; x++) o += t[x], l += s[x], i += t[x] * s[x], h += t[x] * t[x], c += s[x] *
    s[x];
  const u = a * i - o * l,
...
function se(t, s, a, o, l) {
  ...
  return n >= .1 ? B = "strong" : n >= .05 ? B = "moderate" : B = "weak", (F ?? 0) > .03 && y > 0 ?
    M = "momentum" : (F ?? 0) < -.03 && y < 0 ? M = "mean-reversion" : M = "noise", {
      ticker: l,
      rocLookback: s,
      forwardHorizon: a,
      bucketCount: o,
...
function ae(t, s) {
  ...
  return o > .5 ? "bg-emerald-500/80" : o > .15 ? "bg-emerald-500/50" : o > .02 ?
    "bg-emerald-500/25" : o < -.5 ? "bg-red-500/80" : o < -.15 ? "bg-red-500/50" : o < -.02 ?
    "bg-red-500/25" : "bg-muted"
}
...
function L(t) {
  ...
  const s = Math.abs(t);
  return s >= .1 ? t > 0 ? "text-emerald-400" : "text-red-400" : s >= .05 ? t > 0 ?
    "text-emerald-400/70" : "text-red-400/70" : "text-muted-foreground"
}

function xe() {
  const {
    filteredTickersList: t
  } = P(), [s, a] = R.useState([]), [o, l] = R.useState(""), [i, h] = R.useState(21), [c, u] = R
    .useState(21), [m, x] = R.useState(10), [r, k] = R.useState(null), [f, C] = R.useState(!1), [F,
      j
    ] = R.useState(null);
  R.useEffect(() => {
    (async () => {
  ...
  const y = R.useMemo(() => r ? Math.max(...r.buckets.map(n => Math.abs(n.meanFwdReturn)), .001) :
      1, [r]),
    M = R.useCallback(() => {
      if (!r) return;
      const n = ["bucket", "roc_min_pct", "roc_max_pct", "roc_mid_pct", "n",
          "mean_fwd_return_pct", "median_fwd_return_pct", "hit_rate", "std_fwd_return_pct"
        ],
        d = r.buckets.map(w => [w.bucket, w.rocMin.toFixed(4), w.rocMax.toFixed(4), w.rocMid
          .toFixed(4), w.count, w.meanFwdReturn.toFixed(4), w.medianFwdReturn.toFixed(4), w
          .hitRate.toFixed(4), w.stdFwdReturn.toFixed(4)
        ]),
        $ = [...[`# ROC Analysis: ${r.ticker}`, `# ROC lookback: ${r.rocLookback}d`,
          `# Forward horizon: ${r.forwardHorizon}d`, `# Buckets: ${r.bucketCount}`,
          `# Samples: ${r.totalSamples}`, `# Spearman IC: ${S(r.spearmanIC)}`,
          `# Pearson r: ${S(r.pearsonIC)}`, `# Mean fwd return (all): ${p(r.meanFwdAll)}`,
          `# Top - bottom spread: ${p(r.topMinusBottom)}`
        ], n.join(","), ...d.map(w => w.join(","))].join(`
`),
        b = new Blob([$], {
          type: "text/csv"
        }),
      ...
      A.href = N, A.download =
        `roc-analysis-${r.ticker}-roc${r.rocLookback}-fwd${r.forwardHorizon}.csv`, A.click(), URL
        .revokeObjectURL(N)
    }, [r]),
  ...
  return e.jsxs("div", {
    ...
    children: [e.jsxs("div", {
      ...
      children: [e.jsxs("div", {
        ...
        children: ["Sorts every historical day's ROC into ", m,
          " equal-frequency buckets, then reports the average forward ", c,
          "d return inside each bucket. A monotonic up-slope means momentum (high ROC → high forward returns); a down-slope means mean-reversion (high ROC → low forward returns); flat means no signal."
        ]
      })]
    }), e.jsxs("div", {
      ...
      children: [f && e.jsxs("div", {
        ...
        children: [e.jsxs("div", {
          ...
          children: [e.jsx(T, {
            ...
            valueClass: r.topMinusBottom > 0 ? "text-emerald-400" :
              "text-red-400",
            hint: "Mean fwd return of highest-ROC bucket minus lowest-ROC bucket"
          }), e.jsx(T, {
            label: "Verdict",
            value: r.interpretation === "momentum" ?
              `${r.interpretationStrength} momentum` : r
              .interpretation === "mean-reversion" ?
              `${r.interpretationStrength} reversion` : "noise",
            valueClass: r.interpretation === "momentum" ?
              "text-emerald-400" : r.interpretation ===
              "mean-reversion" ? "text-amber-400" :
          ...
          children: [e.jsxs("div", {
            ...
            children: ["Mean Forward ", r.forwardHorizon,
              "d Return by ROC", r.rocLookback, " Bucket"
            ]
          }), e.jsx(le, {
            buckets: r.buckets,
            maxAbsMean: y,
            ...
            children: [e.jsxs("span", {
              ...
              children: [
                "Dashed line = unconditional mean fwd return (",
                p(r.meanFwdAll), ")"
              ]
            }), e.jsxs("span", {
              children: ["Bucket ", r.bucketCount,
                " = highest ROC", r.rocLookback
              ]
            })]
          })]
        }), e.jsx("div", {
          ...
          children: e.jsxs("table", {
            ...
            children: [e.jsx("thead", {
              ...
              children: r.buckets.map(n => {
                const d = n.stdFwdReturn > 0 ? n.meanFwdReturn /
                  n.stdFwdReturn : 0;
                return e.jsxs("tr", {
                  ...
                  children: [e.jsx("td", {
                    ...
                    children: [(n.hitRate * 100)
                      .toFixed(1), "%"
                    ]
                  }), e.jsxs("td", {
                    className: "px-3 py-2 text-right text-muted-foreground",
                    children: [n.stdFwdReturn.toFixed(
                      2), "%"]
                  }), e.jsx("td", {
          ...
          return e.jsxs("div", {
            ...
            children: [e.jsxs("div", {
              ...
              children: [e.jsxs("div", {
                ...
                children: ["ROC: ", p(u.rocMin, 1), " → ", p(u
                  .rocMax, 1)]
              }), e.jsxs("div", {
                children: ["Mean Fwd: ", e.jsx("span", {
                  className: m ? "text-emerald-400" :
                    "text-red-400",
                  children: p(u.meanFwdReturn)
                })]
              }), e.jsxs("div", {
                children: ["Hit Rate: ", (u.hitRate * 100)
                  .toFixed(1), "%"
                ]
              }), e.jsxs("div", {
                children: ["N = ", u.count]
