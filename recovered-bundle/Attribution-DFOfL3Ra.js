...
function Ee(s, o, r, u) {
  ...
  const i = r.close[n],
    ...
    t = r.estimate[n],
    p = r.estimate[m];
  if (!Number.isFinite(i) || !Number.isFinite(g) || !Number.isFinite(d) || !Number.isFinite(l) || !
    Number.isFinite(t) || !Number.isFinite(p)) return null;
...
const M = (s, o = 2) => Number.isFinite(s) ? `${s>=0?"+":""}${s.toFixed(o)}%` : "—",
  ...
  $ = s => !Number.isFinite(s) || s === 0 ? "text-muted-foreground" : s > 0 ? "text-emerald-500" :
  "text-rose-500";

function Ie() {
  const {
    filteredTickersList: s
  } = ye(), o = x.useMemo(() => s.map(c => c.ticker), [s]), [r, u] = x.useState("single"), [n, m] =
    x.useState("auto"), [i, g] = x.useState(""), [d, l] = x.useState(252), [t, p] = x.useState(21),
    [j, S] = x.useState(""), [a, N] = x.useState(null), [y, D] = x.useState("FFO"), [w, R] = x
    .useState(!1), [Z, ee] = x.useState([]), [K, te] = x.useState(!1), [he, V] = x.useState(null), [
      W, pe
    ] = x.useState("multipleShare"), [L, se] = x.useState("desc");
  x.useEffect(() => {
    if (!i && o.length > 0) {
      const b = ["O", "SPG", "PLD", "AMT", "EQIX", "VICI", "WELL"].find(v => o.includes(v)) ??
        o[0];
      g(b)
    }
  }, [o, i]);
  const le = x.useCallback(async () => {
    if (i) {
      ...
      try {
        ...
        n === "auto" && (v.length === 0 || f.length === 0) && (b = "EPS", v = await k(i, F.EPS
          .multiple), f = await k(i, F.EPS.estimate));
        const h = de(c, v, f);
        N(h), D(b)
      } catch (c) {
  ...
  const re = x.useRef({
      ...
      async function T() {
        for (;;) {
          ...
          try {
            ...
            if (n === "auto" && (I.length === 0 || Y.length === 0) && (B = "EPS", I = await k(
                C, F.EPS.multiple), Y = await k(C, F.EPS.estimate)), J.length === 0 || I
              .length === 0 || Y.length === 0) {
              h++, V({
                done: h,
                total: o.length
    ...
    H = x.useMemo(() => {
      ...
      return c.sort((b, v) => {
        ...
        if (typeof f == "string" && typeof h == "string") return L === "asc" ? f
          .localeCompare(h) : h.localeCompare(f);
        const T = f - h;
        return L === "asc" ? T : -T
      }), c
    }, [Z, W, L]),
    ...
    je = () => {
      if (r === "single") {
        ...
        const c = "date,total_ln_pct,multiple_ln_pct,estimate_ln_pct",
          b = P.map(h =>
          `${h.date},${h.total.toFixed(4)},${h.mult.toFixed(4)},${h.est.toFixed(4)}`),
          f = [
            `# ${i} | basis=${y} | window=${d===0?"YTD":`${d}d`} | start=${G?.startDate??""} | end=${G?.endDate??""}`,
            c, ...b
          ].join(`
`);
        ...
        const c =
          "ticker,basis,total_pct,multiple_pct,estimate_pct,multiple_share,estimate_share,same_direction",
          b = H.map(h => [h.ticker, h.basis, h.totalPct.toFixed(4), h.multiplePct.toFixed(4), h
            .estimatePct.toFixed(4), h.multipleShare.toFixed(4), h.estimateShare.toFixed(4), h
            .sameDirection ? "1" : "0"
          ].join(",")),
          f = [
            `# universe attribution | window=${d===0?"YTD":`${d}d`} | basis=${n==="auto"?"auto(FFO->EPS)":n}`,
            c, ...b
          ].join(`
`);
    ...
    children: [e.jsxs("div", {
      ...
      children: [e.jsx("div", {
        ...
        children: S ? e.jsxs("div", {
          ...
          children: ["No data for ", r, ". ", t === "EPS" ? "" :
            "(FFO not available — try forcing EPS basis)"
          ]
        }) : e.jsxs("div", {
          ...
          children: [e.jsxs("div", {
            ...
            children: [e.jsx("div", {
              ...
              children: ["M + E = ", M(l.mult + l.est), " ", e.jsxs(
                "span", {
                  className: "text-muted-foreground",
...
function Oe(s) {
  ...
  return e.jsx("div", {
    ...
    children: m && i ? e.jsxs("div", {
      ...
      children: [
        'Click "Run on N" above to compute the attribution table for the active universe over the ',
        d,
        " window. Each row decomposes the ticker's total log-return into multiple-expansion vs estimate-revision contributions."
      ]
    }) : e.jsxs("table", {
...
function _e({
  ...
  return x.useEffect(() => {
    ...
    return l(), () => {
      const t = r.current;
      t?.__ro && t.__ro.disconnect(), r.current?.remove(), r.current = null, u.current = null,
        n.current = null, m.current = null
    }
  }, []), x.useEffect(() => {
