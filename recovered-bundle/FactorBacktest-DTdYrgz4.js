...
function Nt(r) {
  ...
  for (const a of L) {
    ...
    l == null || c == null || !Number.isFinite(l) || !Number.isFinite(c) || l <= 0 ? s[a.label] =
      null : s[a.label] = c / l - 1
  }
  return s
}
...
function st(r, s, a, l, c) {
  ...
  return l && (d.meanReturnSR = l.length ? T(l) : NaN, d.hitRateSR = l.length ? l.filter(g => g >=
    c).length / l.length : NaN), d
}
...
function Mt(r, s) {
  ...
  for (const o of l) {
    const h = a.get(o),
      R = s.rankingMode ?? "peg",
      p = h.filter(i => !(i.mult == null || i.grw == null || !Number.isFinite(i.mult) || !Number
        .isFinite(i.grw) || !(i.mult > 0) || i.closeAtT == null || !(i.closeAtT > 0) || R ===
        "peg" && !(i.grw > 0)));
    ...
    for (let i = 0; i < p.length; i++) {
      ...
      for (const w of L) {
        const Q = O[w.label];
        if (Q != null && Number.isFinite(Q) && (c[S][w.label].push(Q), S === "Q1" && k[o][w.label]
            .q1.push(Q), S === "Q5" && k[o][w.label].q5.push(Q)), z) {
          const V = z[w.label];
  ...
  for (const o of ["Q1", "Q2", "Q3", "Q4", "Q5"])
    for (const h of L) v.push(st(o, h.label, c[o][h.label], s.sectorRelative ? d[o][h.label] : null,
      s.threshold));
  const E = [];
  for (const o of ["BR", "BL", "TR", "TL"])
    for (const h of L) E.push(st(o, h.label, g[o][h.label], s.sectorRelative ? f[o][h.label] : null,
      s.threshold));
  const x = {
    "1M": [],
    "3M": [],
  ...
  const _ = L.map(o => ({
      ...
      hitRate: x[o.label].length ? x[o.label].filter(h => h >= s.threshold).length / x[o.label]
        .length : NaN
    })),
    P = L.map(o => {
...
function le(r, s = .1) {
  ...
  return a >= 0 ? `rgba(34, 197, 94, ${Math.min(.55,a*.55)})` :
    `rgba(239, 68, 68, ${Math.min(.55,-a*.55)})`
}
...
function Ct() {
  const r = dt(),
    ...
    I = m.useCallback(t => {
      !t || typeof t != "object" || (typeof t.metricY == "string" && a(t.metricY), typeof t
        .metricX == "string" && c(t.metricX), typeof t.startDate == "string" && g(t.startDate),
        typeof t.endDate == "string" && k(t.endDate), typeof t.rebalanceDays == "number" &&
        Number.isFinite(t.rebalanceDays) && A(t.rebalanceDays), typeof t.thresholdPct ==
        "string" && E(t.thresholdPct), typeof t.sectorRelative == "boolean" && _(t
          .sectorRelative), (t.rankingMode === "peg" || t.rankingMode === "zscore") && C(t
          .rankingMode))
    }, []);
  ...
  const ce = m.useMemo(() => {
    ...
    Z = m.useMemo(() => {
      let t = re;
      return r.universeTickers && (t = t.filter(n => r.universeTickers.has(n.ticker))), t.map(n =>
        n.ticker)
      ...
      try {
        ...
        async function lt() {
          for (;;) {
            ...
            const N = oe[j];
            try {
              const [F, W, ee] = await Promise.all([Te(N, "close"), Te(N, l), Te(N, s)]), ue =
                new Map;
              for (const q of F) q.value != null && Number.isFinite(q.value) && ue.set(q.time,
                q.value);
              ae.set(N, ue);
              const he = new Map;
              for (const q of W) q.value != null && Number.isFinite(q.value) && he.set(q.time,
                q.value);
              Y.set(N, he);
              const se = new Map;
              for (const q of ee) q.value != null && Number.isFinite(q.value) && se.set(q
                .time, q.value);
              Ee.set(N, se)
            } catch {}
            X(F => ({
        ...
        if (Se.length === 0) {
          const j = H.length * oe.length,
            N = [];
          throw Fe.size === 0 && N.push(
            `Y metric "${s}" returned no data for any of ${oe.length} tickers — check that this metric exists for your universe.`
            ), we.size === 0 && N.push(
            `X metric "${l}" returned no data for any of ${oe.length} tickers — check that this metric exists for your universe.`
            ), N.length === 0 && N.push(
            `Joined 0 of ${j.toLocaleString()} candidate (date×ticker) pairs. Missing X: ${Oe.toLocaleString()}, missing Y: ${$e.toLocaleString()}, failed normalize: ${Ae.toLocaleString()}, no close series: ${_e.toLocaleString()}, no close on rebal date: ${Ge.toLocaleString()}. Tickers with any X: ${we.size}, with any Y: ${Fe.size}, with any close: ${Ie.size}.`
            ), new Error(N.join(" "))
        }
        const qe = Mt(Se, {
          threshold: ne,
    ...
    S = m.useCallback(() => {
      ...
      ve(`factor_bt_quintile_${s}_${l}.csv`, ke(t, ["bucket", "horizon", "n", "meanReturn",
        "medianReturn", "stdReturn", "hitRate", "meanReturnSR", "hitRateSR"
      ]))
    }, [u, l, s]),
    O = m.useCallback(() => {
      if (!u) return;
      ...
      ve(`factor_bt_quadrant_${s}_${l}.csv`, ke(t, ["bucket", "horizon", "n", "meanReturn",
        "medianReturn", "stdReturn", "hitRate", "meanReturnSR", "hitRateSR"
      ]))
    }, [u, l, s]),
    z = m.useCallback(() => {
  ...
  return e.jsxs("div", {
    ...
    children: [e.jsxs("div", {
      ...
      children: [B && e.jsx(Ue, {
        ...
        children: [G.message, G.total > 0 && G.stage !== "done" && G.stage !==
          "error" && e.jsxs(e.Fragment, {
        ...
        children: [e.jsx(xt, {
          ...
          children: [e.jsx("div", {
            ...
            children: ["Tests whether stocks in the ", e.jsx("span", {
              ...
              " (low Y multiple + high X growth) generate higher forward returns. ",
              e.jsx("span", {
                className: "font-mono",
                children: "Q1"
              }),
              " = cheapest PEG (mult ÷ growth) ⇒ expected outperformer. ",
              e.jsx("span", {
                className: "font-mono",
                children: "BR"
              }),
        ...
        children: [e.jsxs("div", {
            ...
            children: [e.jsxs("div", {
              ...
              children: ["Rank: ", e.jsx("span", {
                className: "text-foreground",
                children: (u.rankingMode ?? "peg") === "zscore" ?
                  "Z-score" : "PEG"
              })]
            }), e.jsxs("div", {
                ...
                children: [e.jsx("thead", {
                  ...
                  children: Me.map(t => {
                    const n = u.quintile.icMeans[t],
                      ...
                      n / b : null,
                      ne = n == null ? "—" : n >= .05 ?
                      "Strong predictive (cheap PEG → higher ret)" :
                      n >= .02 ? "Mild predictive" : n >= -
                      .02 ? "Noise" : n >= -.05 ?
                      "Mild contrarian" :
                      "Strong contrarian (cheap PEG → lower ret)";
                    return e.jsxs("tr", {
          ...
          }), Q("Quintile Buckets (Q1 = cheapest PEG = lowest mult/grw)", $, V,
          S), e.jsxs("div", {
            className: "border border-border rounded bg-card/40",
            children: [e.jsx("div", {
              className: "px-3 py-2 border-b border-border font-semibold text-sm",
              ...
              children: e.jsxs("table", {
                ...
                children: [e.jsx("thead", {
                  ...
                  children: u.quintile.longShort.map(t => e
                    .jsxs("tr", {
                      className: "border-b border-border/50",
                      children: [e.jsx("td", {
                        className: "px-2 py-1.5 font-mono",
          ...
          }), Q("Quadrant Buckets (BR = LowMult + HighGrowth — your target)", de,
            J, O), e.jsxs("div", {
            className: "border border-border rounded bg-card/40",
            children: [e.jsx("div", {
              className: "px-3 py-2 border-b border-border font-semibold text-sm",
              ...
              children: e.jsxs("table", {
                ...
                children: [e.jsx("thead", {
                  ...
                  children: u.quadrant.brMinusRest.map(t => e
                    .jsxs("tr", {
                      className: "border-b border-border/50",
                      children: [e.jsx("td", {
                        className: "px-2 py-1.5 font-mono",
