...
function ge(h) {
  const d = Math.abs(h);
  return d >= 2.5 ? h > 0 ? "#ef4444" : "#22c55e" : d >= 2 ? h > 0 ? "#f97316" : "#4ade80" : d >=
    1.5 ? h > 0 ? "#fbbf24" : "#6ee7b7" : "#94a3b8"
}

function at(h) {
  const d = Math.abs(h);
  return d >= 2.5 ? h > 0 ? "rgba(239,68,68,0.12)" : "rgba(34,197,94,0.12)" : d >= 2 ? h > 0 ?
    "rgba(249,115,22,0.08)" : "rgba(74,222,128,0.08)" : "transparent"
}
    ...
    l.push(g), v.push(N), E.current = l, p.current = v, l.forEach((i, A) => {
      i.timeScale().subscribeVisibleLogicalRangeChange(f => {
        ...
        F.current || (F.current = !0, l.forEach((M, Y) => {
          if (Y !== A) try {
            f.time ? M.setCrosshairPosition(NaN, f.time, v[Y]) : M
...
function mt() {
  const [d, T] = o.useState("close"), [s, w] = o.useState("252"), [x, E] = o.useState(2), [p, F] = o
    .useState("zscore"), [n, S] = o.useState(!0), [l, v] = o.useState(null), [j, k] = o.useState(
      "all"), [g, N] = o.useState(0), [m, i] = o.useState(""), A = o.useRef(null), f = () => ({
      min: "",
      max: ""
    }), [M, Y] = o.useState(f()), [O, X] = o.useState(f()), [z, J] = o.useState(f()), [V, B] = o
    .useState(f()), [I, ee] = o.useState(f()), [c, R] = o.useState(f()), te = [M, O, z, V, I, c]
    .some(t => t.min.trim() !== "" || t.max.trim() !== ""), se = () => {
      ...
      t?.metric && T(t.metric), t?.lookback && w(t.lookback), t?.zThreshold && E(t.zThreshold), t
        ?.sortBy && F(t.sortBy), t?.showZScore !== void 0 && S(t.showZScore), t?.filterMode && k(t
          .filterMode), t?.searchQuery !== void 0 && i(t.searchQuery), t?.vfZScore && Y(t
          .vfZScore), t?.vfRatio && X(t.vfRatio), t?.vfChg30 && J(t.vfChg30), t?.vfChg90 && B(t
          .vfChg90), t?.vfMean && ee(t.vfMean), t?.vfStd && R(t.vfStd)
    }, []), Ae = tt();
  Ze("pair-ratios", Re, Ee, {
    universeSig: Ae,
    resultFields: ["searchQuery"]
  });
  const Pe = new Set(["close", "P/E LTM", "P/E FY2", "P/S LTM", "P/S FY2", "EV/EBITDA LTM",
      "EV/EBITDA FY2", "P/FFO LTM", "P/FFO FY2", "P/AFFO LTM", "P/AFFO FY2", "Implied Cap Rate",
    ...
    K = o.useMemo(() => (oe ? be : ve).filter(a => a.ticker !== "TEST" && a.ticker !== "TST2").map(
      a => a.ticker).sort(), [oe, be, ve]),
    {
      data: _
    } = Se({
    ...
    G = o.useMemo(() => !W || W.size < 2 || !_ || _.length === 0 ? [] : lt(K, W, _, d, s), [K, W, _,
      d, s, x
    ]),
    Z = o.useMemo(() => {
      let t = [...G];
      ...
      switch (t = a(t, M, r => r.zScore), t = a(t, O, r => r.currentRatio), t = a(t, z, r => r
          .pctChange30d), t = a(t, V, r => r.pctChange90d), t = a(t, I, r => r.mean), t = a(t, c,
          r => r.std), p) {
        case "zscore":
          t.sort((r, u) => Math.abs(u.zScore) - Math.abs(r.zScore));
          break;
        case "name":
          t.sort((r, u) => `${r.tickerA}/${r.tickerB}`.localeCompare(
          `${u.tickerA}/${u.tickerB}`));
          break;
        case "change30":
    ...
    $ = o.useMemo(() => {
      ...
      return a.length === 0 ? Z : a.length === 1 ? Z.filter(r => r.tickerA.includes(a[0]) || r
        .tickerB.includes(a[0])) : Z.filter(r => r.tickerA.includes(a[0]) && r.tickerB.includes(
        a[1]) || r.tickerA.includes(a[1]) && r.tickerB.includes(a[0]))
    }, [Z, m]);
  o.useEffect(() => {
    N(0)
  ...
  o.useEffect(() => {
    const t = a => {
      a.key === "/" && !a.ctrlKey && !a.metaKey && document.activeElement?.tagName !==
        "INPUT" && (a.preventDefault(), A.current?.focus()), a.key === "Escape" && document
        .activeElement === A.current && (i(""), A.current?.blur())
    };
    return document.addEventListener("keydown", t), () => document.removeEventListener(
      "keydown", t)
  }, []);
  const ze = o.useCallback(() => {
      const t = "Pair,Current Ratio,Z-Score,Mean,Std Dev,30d Chg%,90d Chg%",
        a = Z.map(C =>
          `${C.tickerA}/${C.tickerB},${C.currentRatio.toFixed(4)},${C.zScore.toFixed(3)},${C.mean.toFixed(4)},${C.std.toFixed(4)},${C.pctChange30d.toFixed(2)},${C.pctChange90d.toFixed(2)}`
          ),
        r = [t, ...a].join(`
`),
        u = new Blob([r], {
      ...
      children: [e.jsxs("div", {
        ...
        children: [e.jsxs("div", {
          ...
          children: [e.jsxs("div", {
            ...
            children: [Z.length.toLocaleString(),
              " pairs match filters"]
          })]
        }), e.jsxs("div", {
          ...
          children: [e.jsx("div", {
            ...
            children: [e.jsxs("div", {
              ...
              children: ["Above +", x, "σ:", " ", e.jsx("span", {
                ...
                children: G.filter(t => t.zScore >= x)
                  .length
              })]
            }), e.jsxs("div", {
      ...
      children: Ye ? e.jsxs("div", {
        ...
        children: [e.jsxs("div", {
          ...
          children: [e.jsx(Ue, {
            ...
            children: [$.length.toLocaleString(), " match", $.length !==
              1 ? "es" : ""
            ]
          })]
        }), e.jsxs("div", {
          ...
          children: [e.jsxs("table", {
            ...
            children: [e.jsx("thead", {
              ...
              children: Le.map((t, a) => e.jsxs("tr", {
                ...
                children: [e.jsxs("td", {
                  ...
                  children: e.jsxs("span", {
                    style: {
                      color: t.pctChange30d > 0 ?
                        "#22c55e" : t.pctChange30d <
                        0 ? "#ef4444" : "#94a3b8"
                    },
                    children: [t.pctChange30d >= 0 ?
                      "+" : "", t.pctChange30d
                      .toFixed(1), "%"
                    ]
                  })
                  ...
                  children: e.jsxs("span", {
                    style: {
                      color: t.pctChange90d > 0 ?
                        "#22c55e" : t.pctChange90d <
                        0 ? "#ef4444" : "#94a3b8"
                    },
                    children: [t.pctChange90d >= 0 ?
                      "+" : "", t.pctChange90d
                      .toFixed(1), "%"
                    ]
                  })
            ...
            children: [e.jsxs("span", {
              className: "text-muted-foreground",
              children: ["Showing ", g * 100 + 1, "–", Math.min((g +
                  1) * 100, $.length), " of ", $.length
            ...
            children: m.trim() ? `No pairs matching "${m.trim()}"` : j ===
              "extreme" ? `No pairs beyond ±${x}σ` :
...
function ct({
  ...
  return o.useEffect(() => {
    ...
    const p = ot(h.zScoreSeries, it),
      ...
      j = v - 2 / F * (l / 2),
      k = v + 2 / F * (l / 2);
    s.fillStyle = "rgba(239,68,68,0.06)", s.fillRect(n, n, S, j - n), s.fillStyle =
      "rgba(34,197,94,0.06)", s.fillRect(n, k, S, E - n - k), s.strokeStyle =
      "rgba(255,255,255,0.1)", s.lineWidth = .5, s.beginPath(), s.moveTo(n, v), s.lineTo(x - n,
        v), s.stroke(), s.strokeStyle = "rgba(255,255,255,0.08)", s.setLineDash([2, 2]), s
      .beginPath(), s.moveTo(n, j), s.lineTo(x - n, j), s.stroke(), s.beginPath(), s.moveTo(n,
        k), s.lineTo(x - n, k), s.stroke(), s.setLineDash([]), s.strokeStyle = "#0ea5e9", s
      .lineWidth = 1, s.beginPath();
