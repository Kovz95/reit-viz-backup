    ...
    if (t.sigmaBasis === "full") {
      ...
      for (let g = 1; g < o; g++) {
        ...
        const v = (m - x) / i,
          a = Math.abs(v);
        let N = !1;
        t.sigmaDirection === "either" ? N = a >= t.sigma : t.sigmaDirection === "up" ? N = v >= t
          .sigma : N = v <= -t.sigma, N && (u[g] = !0, p[g] = m)
      }
    } else {
      ...
      for (let x = r + 1; x < o; x++) {
        ...
        const a = (v - g) / m,
          N = Math.abs(a);
        let b = !1;
        t.sigmaDirection === "either" ? b = N >= t.sigma : t.sigmaDirection === "up" ? b = a >= t
          .sigma : b = a <= -t.sigma, b && (u[x] = !0, p[x] = v)
      }
    }
    ...
    for (let r = 252; r < o; r++) {
      ...
      for (let d = 1; d < o; d++) {
        ...
        const i = (x / r - 1) * 100,
          g = Math.abs(i);
        let m = !1;
        t.gapDirection === "either" ? m = g >= t.gapPct : t.gapDirection === "up" ? m = i >= t
          .gapPct : m = i <= -t.gapPct, m && (u[d] = !0, p[d] = i)
      }
  return {
    mask: u,
    value: p
...
function Ae({
  ...
  const r = d.map(f => Lt(f, t, n)),
    x = d.map(f => f.type === "sigma" ? f.sigmaBasis === "full" ? 30 : f.sigmaWindow + 2 : f
      .type === "high52" || f.type === "low52" ? 253 : f.type === "gap" ? 2 : 0),
...
function Kt(t) {
  ...
  return t.type === "high52" ? "New 52w high" : t.type === "low52" ? "New 52w low" : t.type ===
    "gap" ? `${t.gapDirection==="either"?"±":t.gapDirection==="up"?"+":"−"}${t.gapPct}% gap` : ""
}
...
function as() {
  const {
    data: t = []
      ...
      filteredCount: u,
      totalCount: p
    } = yt(), d = C.useMemo(() => c ? t.filter(s => c.has(s.ticker)) : t, [t, c]), [r, x] = C
    .useState("single"), {
      baskets: i
    } = Ye(), g = nt(), [m, v] = C.useState("");
  C.useEffect(() => {
    !m && d.length && v(d[0].ticker), m && d.length && !d.find(s => s.ticker === m) && v(d[0]
      .ticker)
  }, [d, m]);
  const [a, N] = C.useState("");
  C.useEffect(() => {
    !a && d.length >= 2 && N(d[1].ticker), a && d.length && !d.find(s => s.ticker === a) && N(d
      .find(s => s.ticker !== m)?.ticker ?? "")
  }, [d, a, m]);
  const [b, R] = C.useState([ze("sigma")]), [f, F] = C.useState("AND"), [y, A] = C.useState(20), [S,
      W
    ] = C.useState("edge"), [D, H] = C.useState("desc"), [h, re] = C.useState(null), [_, ne] = C
    .useState(null), [Z, U] = C.useState(null), [he, Ie] = C.useState(!1), [rt, lt] = C.useState(
      null), [Be, Te] = C.useState(!1), {
      data: ke,
      isFetching: ot
    } = be({
  ...
  const pe = C.useMemo(() => h ? h.conditions.map(Kt).join(h.combinator === "AND" ? " AND " :
      " OR ") : "", [h]),
    mt = () => {
      if (!oe || !h) return;
      const s = ["Date,TriggerValue," + B.map(l => `Fwd_${l}d_%`).join(","), ...oe.events.map(l => [
        l.date, l.triggerValue.toFixed(4), ...B.map(j => {
          const k = l.fwd[j];
...
`))
    ...
    ut = () => {
      ...
      const s = ["Date,RatioTriggerValue," + B.map(l => `Fwd_${l}d_%`).join(","), ...ce.events.map(
        l => [l.date, l.triggerValue.toFixed(4), ...B.map(j => {
          const k = l.fwd[j];
          return k == null ? "" : k.toFixed(4)
        })].join(","))];
...
`))
    ...
    xt = () => {
      if (!Z || !h) return;
      const l = [
        ["Ticker", "Name", "Events", ...B.flatMap(j => [`Mean_${j}d`, `Pup_${j}d`,
          `Edge_${j}d_pp`])].join(",")
      ];
      for (const j of Z) {
        const k = [j.ticker, `"${j.name.replace(/"/g,'""')}"`, String(j.events), ...B.flatMap(M => {
          ...
          return [Number.isFinite(T.mean) ? T.mean.toFixed(4) : "", Number.isFinite(T.pUp) ? T
            .pUp.toFixed(4) : "", Number.isFinite(w) ? w.toFixed(4) : ""
          ]
        })];
        l.push(k.join(","))
      }
...
`))
    ...
    ht = () => {
      ...
      for (const j of Z)
        for (const k of j.eventRows) l.push([j.ticker, `"${j.name.replace(/"/g,'""')}"`, k.date,
          Number.isFinite(k.triggerValue) ? k.triggerValue.toFixed(4) : "", ...B.map(M => k.fwd[
            M] != null ? k.fwd[M].toFixed(4) : "")
        ].join(","));
...
`))
      ...
      return s.sort((l, j) => {
        ...
        switch (S) {
          ...
          case "events":
            ...
            break;
          case "mean":
            w = Number.isFinite(k.mean) ? k.mean : -1 / 0, E = Number.isFinite(M.mean) ? M
              .mean : -1 / 0;
            break;
          case "pUp":
            w = Number.isFinite(k.pUp) ? k.pUp : -1 / 0, E = Number.isFinite(M.pUp) ? M.pUp :
              -1 / 0;
            break;
          case "edge": {
            const z = Number.isFinite(k.mean) && Number.isFinite(T.mean) ? k.mean - T.mean : -
              1 / 0,
              O = Number.isFinite(M.mean) && Number.isFinite($.mean) ? M.mean - $.mean : -1 /
              0;
            w = z, E = O;
            break
          }
    ...
    Ve = C.useMemo(() => {
      ...
      return s.length ? B.map(l => {
        const j = s.map($ => $.stats.find(w => w.horizon === l)?.mean).filter($ => Number
            .isFinite($)),
          ...
          M = s.map($ => {
            ...
            return w && E && Number.isFinite(w.mean) && Number.isFinite(E.mean) ? w.mean - E
              .mean : NaN
  ...
  return e.jsxs("div", {
    ...
    children: [e.jsxs(te, {
      children: [e.jsx(ae, {
        ...
        children: [e.jsxs("div", {
          ...
          children: [e.jsxs("div", {
            ...
            children: [e.jsxs(X, {
                onClick: we,
                disabled: r === "single" && !m || r === "pairs" && (
                    !m || !a || m === a) || he || r === "cross" && d
                  .length === 0,
                "data-testid": "button-run-study",
                className: "h-8",
                children: [e.jsx(It, {
                    className: "w-3.5 h-3.5 mr-1.5"
                  }), r === "cross" ? `Run on ${d.length}` : r ===
                  "pairs" ? "Run pair study" : "Run study"
                ]
              }), h?.mode === "single" && oe && oe.events.length >
              0 && e.jsxs(X, {
                variant: "outline",
                onClick: mt,
              ...
              }), h?.mode === "pairs" && ce && ce.events.length > 0 &&
              e.jsxs(X, {
                variant: "outline",
                onClick: ut,
          ...
          children: [e.jsxs("div", {
            ...
            children: b.map((s, l) => e.jsxs("div", {
              ...
              children: [b.length > 1 && l > 0 && e.jsx("span", {
                ...
                children: e.jsx(Ht, {
                  cond: s,
                  onChange: j => R(b.map(k => k.id === s
                    .id ? j : k)),
                  onRemove: () => R(b.filter(j => j
                    .id !== s.id)),
      ...
      displaySymbol: g(m),
      tickerName: Se(m) ?
        `${i.find(s=>`BASKET:${s.id}`===m)?.tickers.length??0} tickers` : d.find(s => s
          .ticker === m)?.name ?? t.find(s => s.ticker === m)?.name ?? "",
      ...
      children: e.jsxs(se, {
        ...
        children: [e.jsxs("div", {
          className: "flex-1",
          children: [he ? "Running event study across universe…" : "Complete",
            _ && e.jsxs("span", {
      ...
      children: [e.jsxs("div", {
        ...
        children: [e.jsx(ue, {
          ...
          className: "text-xs text-muted-foreground",
          children: [q.filter(s => s.events > 0).length,
            " tickers had at least one match"
          ]
        })]
      }), Ve && e.jsxs(te, {
        children: [e.jsx(ae, {
            ...
            children: e.jsxs("table", {
              ...
              children: [e.jsx("thead", {
                ...
                children: Ve.map(s => e.jsxs("tr", {
                  ...
                  children: [e.jsx("td", {
                    className: "py-1.5 pr-3 font-medium",
                    children: s.horizon === 1 ?
                      "1 day" : `${s.horizon} days`
                  }), e.jsx("td", {
                    ...
                    children: Number.isFinite(s
                        .avgPup) ?
                      `${(s.avgPup*100).toFixed(1)}%` :
                      "—"
                  }), e.jsx("td", {
                    ...
                    children: Number.isFinite(s
                        .avgEdge) ?
                      `${s.avgEdge>0?"+":""}${s.avgEdge.toFixed(2)} pp` :
                      "—"
                  }), e.jsx("td", {
                    className: "py-1.5 pr-3",
                    children: Number.isFinite(s
                        .posEdgeFraction) ?
                      `${(s.posEdgeFraction*100).toFixed(0)}%` :
                      "—"
                  })]
                }, s.horizon))
              })]
          ...
          children: [e.jsxs("div", {
            ...
            children: [e.jsxs("table", {
              ...
              children: [e.jsx("thead", {
                ...
                children: q.map(s => {
                  const l = s.stats.find(P => P
                      .horizon === y),
                    j = s.baseline.find(P => P
                      .horizon === y),
                    k = Number.isFinite(l.mean) &&
                    Number.isFinite(j.mean) ? l.mean - j
                    .mean : NaN,
                    M = Number.isFinite(k) ? k > 0 ?
                    "text-chart-2" :
                    "text-destructive" : "",
                    T = Number.isFinite(l.mean) ? l
                    .mean > 0 ? "text-chart-2" :
                    "text-destructive" : "",
                    $ = Number.isFinite(l.pUp) ? l
                    .pUp >= .5 ? "text-chart-2" :
                    "text-destructive" : "",
                    w = h.conditions[0],
                    E = w && (w.type === "sigma" || w
    ...
    children: [e.jsxs("div", {
      ...
      children: [e.jsx(ae, {
        ...
        children: [e.jsx("div", {
          ...
          children: e.jsxs("table", {
            ...
            children: [e.jsx("thead", {
              ...
              children: n.stats.map(i => {
                const g = n.baseline.find(N => N.horizon === i
                    .horizon),
                  m = Number.isFinite(i.mean) && Number
                  .isFinite(g.mean) ? i.mean - g.mean : NaN,
                  v = Number.isFinite(m) ? m > 0 ?
                  "text-chart-2" : "text-destructive" : "",
                  a = Number.isFinite(i.pUp) ? i.pUp >= .5 ?
                  "text-chart-2" : "text-destructive" : "";
                return e.jsxs("tr", {
                  className: "border-b border-border/50 hover:bg-muted/30",
                  children: [e.jsx("td", {
                    className: "py-1.5 pr-3 font-medium",
                    children: i.horizon === 1 ?
                      "1 day" : `${i.horizon} days`
                  }), e.jsx("td", {
                    className: "py-1.5 pr-3",
                    children: i.count
                  }), e.jsx("td", {
                    ...
                    children: Number.isFinite(i.pUp) ?
                      `${(i.pUp*100).toFixed(1)}%` :
                      "—"
                  }), e.jsx("td", {
                    className: "py-1.5 pr-3",
                    ...
                    className: "py-1.5 pr-3 text-muted-foreground",
                    children: [V(i.p25), " / ", V(i
                      .p75)]
                  }), e.jsxs("td", {
                    className: "py-1.5 pr-3 text-muted-foreground",
                    children: [V(i.min), " / ", V(i
                      .max)]
                  }), e.jsx("td", {
                    className: "py-1.5 pr-3 text-muted-foreground",
                    children: V(g.mean)
                    ...
                    className: "py-1.5 pr-3 text-muted-foreground",
                    children: Number.isFinite(g.pUp) ?
                      `${(g.pUp*100).toFixed(1)}%` :
                      "—"
                  }), e.jsx("td", {
                    className: `py-1.5 pr-3 font-semibold ${v}`,
                    children: Number.isFinite(m) ?
                      `${m>0?"+":""}${m.toFixed(2)} pp` :
                      "—"
                  })]
                }, i.horizon)
          ...
          children: ["Returns are close-to-close from the event bar. ", e.jsx(
              "b", {
                children: "Edge vs. base"
              }), " subtracts the unconditional mean at each horizon. ", e
            .jsx("b", {
              children: "Win/Loss"
        ...
        children: e.jsx(_e, {
          ...
          children: e.jsxs(Dt, {
            ...
            children: [e.jsx(Bt, {
              ...
              formatter: (i, g) => g === "cumret" ? [
                `${Number(i).toFixed(3)}%`, "Avg cum ret"
              ] : [i, g],
            ...
            children: e.jsx(_e, {
              ...
              children: e.jsxs(Vt, {
                ...
                children: [e.jsx(Pe, {
                  ...
                  interval: Math.max(1, Math.floor(g.length /
                    6))
                }), e.jsx(Re, {
                  tick: {
                    fontSize: 9,
                  ...
                  labelFormatter: (m, v) => {
                    const a = v?.[0]?.payload;
                    return a ?
                      `${a.lo.toFixed(2)}% to ${a.hi.toFixed(2)}%` :
                      m
                  }
                }), e.jsx(qe, {
                  x: g.find(m => m.lo <= 0 && m.hi >= 0)
                    ?.bucket ?? "0",
                  stroke: "hsl(var(--muted-foreground))"
                }), e.jsx(Xe, {
                  dataKey: "count",
          ...
          children: [e.jsxs(X, {
            ...
            onClick: () => {
              ...
              et({
                ticker: r ? p : t.symbol,
                label: r ?
                  `Pair ${p}/${d} · ${n.events.length} signals · ${c}` :
                  `Price Action · ${n.events.length} signals · ${c}`,
                signals: i
              })
            },
        ...
        children: e.jsxs("div", {
          ...
          children: [e.jsxs("table", {
            ...
            children: [e.jsx("thead", {
              ...
              children: (o ? n.events : n.events.slice(-200))
              .slice().reverse().map((i, g) => {
            ...
            children: ["Showing most recent 200 of ", n.events.length,
              " signals. Toggle “Show all” above or use CSV export for the full list."
            ]
          })]
        })
      })]
...
function Ee(t) {
  return Number.isFinite(t) ? t >= 3 ? "bg-destructive/20 text-destructive border-destructive/40" :
    t >= 2 ? "bg-chart-3/20 text-chart-3 border-chart-3/40" : t >= 1 ?
    "bg-chart-1/20 text-chart-1 border-chart-1/40" :
    "bg-muted text-muted-foreground border-border" : "bg-muted text-muted-foreground"
}
    ...
    children: [e.jsx(ae, {
      ...
      children: e.jsxs(ie, {
        ...
        children: [e.jsx(tt, {
          ...
          children: ["What sigma is this move? (μ/σ basis:", " ", u === "full" ?
            "full history" : `rolling ${N?.window??o}d`, ")"
          ]
        })]
      })
    }), e.jsxs(se, {
      ...
      children: [N ? e.jsxs("div", {
        ...
        children: [e.jsx(Ge, {
          ...
          children: [e.jsx("div", {
            ...
            className: "text-[10px] text-muted-foreground",
            children: [
              "Type a % move (positive or negative) to see what sigma it represents for ",
              d, " today."
            ]
          })]
        })]
  ...
  return e.jsxs("div", {
    ...
    children: [e.jsxs("div", {
      ...
      children: [e.jsx("div", {
        ...
        className: "text-[10px] text-muted-foreground",
        children: c === "full" ?
          `full-history μ=${t.toFixed(2)}%, σ=${n.toFixed(2)}% over ${u} days` :
          `rolling μ=${t.toFixed(2)}%, σ=${n.toFixed(2)}% over last ${o}d`
      })]
    }), e.jsx("div", {
      className: "overflow-auto",
      children: e.jsxs("table", {
...
function it({
  ...
  const o = `${t.z>=0?"+":""}${t.z.toFixed(2)}σ`,
    ...
    p = t.oneInNDays >= 252 ? `1 in ~${(t.oneInNDays/252).toFixed(1)} years` : t.oneInNDays >= 21 ?
    `1 in ~${(t.oneInNDays/21).toFixed(1)} months` : `1 in ~${Math.round(t.oneInNDays)} days`;
  return e.jsxs("div", {
    ...
    children: [e.jsxs("div", {
      ...
      className: "text-[10px] text-muted-foreground",
      children: [c === "full" ? "Full-history" : "Rolling", " μ=", t.mu.toFixed(2),
        "%, σ=", t.sigma.toFixed(2), "%", c === "full" ? ` over ${t.totalDays} days` :
        ` over last ${t.window}d`
      ]
    }), e.jsxs("div", {
      className: "text-[10px] text-muted-foreground",
...
function Xt({
  ...
  const [u, p] = C.useState(null), [d, r] = C.useState(!1), [x, i] = C.useState("absSigma"), g = C
    .useCallback(async () => {
      ...
      const a = u.slice();
      return a.sort((N, b) => {
        const R = x === "absSigma" ? N.sigma?.absZ ?? -1 / 0 : x === "sigma" ? N.sigma?.z ?? -
          1 / 0 : x === "absPct" ? Math.abs(N.pct) : N.pct;
        return (x === "absSigma" ? b.sigma?.absZ ?? -1 / 0 : x === "sigma" ? b.sigma?.z ?? -
          1 / 0 : x === "absPct" ? Math.abs(b.pct) : b.pct) - R
      }), a
    ...
    children: [e.jsxs(ae, {
      ...
      children: [e.jsxs(ie, {
        ...
        children: [e.jsx(tt, {
          ...
          className: "text-[11px] font-normal text-muted-foreground",
          children: ["Ranks ", t.length,
            " tickers by the sigma of their latest move (μ/σ basis:", " ",
            c === "full" ? "full history" : `rolling ${n}d`, ")"
          ]
        })]
      }), e.jsxs("div", {
        className: "flex items-center gap-2",
      ...
      children: m ? m.length === 0 ? e.jsx("div", {
        ...
        children: e.jsxs("table", {
          ...
          children: [e.jsx("thead", {
            ...
            children: m.map(a => {
              const N = !Number.isFinite(a.pct) || a.pct === 0 ? "" : a
                .pct > 0 ? "text-chart-2" : "text-destructive",
                b = !Number.isFinite(a.gap) || a.gap === 0 ? "" : a
                .gap > 0 ? "text-chart-2" : "text-destructive",
                R = a.sigma ? (a.sigma.percentileAbs * 100).toFixed(1) +
                "%" : "—",
                f = a.sigma ? a.sigma.oneInNDays >= 252 ?
                `1/${(a.sigma.oneInNDays/252).toFixed(1)}y` : a.sigma
                .oneInNDays >= 21 ?
                `1/${(a.sigma.oneInNDays/21).toFixed(1)}mo` :
                `1/${Math.round(a.sigma.oneInNDays)}d` : "—";
              return e.jsxs("tr", {
                ...
                children: [e.jsx("td", {
                  ...
                  children: Number.isFinite(a.pct) ?
                  ...
                  children: a.sigma ? e.jsxs("span", {
                    ...
                    children: [a.sigma.z >= 0 ? "+" : "",
                      a.sigma.z.toFixed(2), "σ"
                    ]
                  }) : "—"
                }), e.jsx("td", {
                  className: "py-1 pr-3 text-muted-foreground",
                  ...
                  className: `py-1 pr-3 ${b}`,
                  children: Number.isFinite(a.gap) ?
                  ...
                  children: a.gapSigma ? e.jsxs("span", {
                    ...
                    children: [a.gapSigma.z >= 0 ? "+" :
                      "", a.gapSigma.z.toFixed(2), "σ"
                    ]
                  }) : "—"
                })]
              }, a.ticker)
        ...
        children: ["Click ", e.jsx("span", {
            className: "font-semibold",
            children: "Load moves"
          }), " to compute the latest daily move and sigma across ", t.length,
          " tickers."
        ]
      })
    })]
