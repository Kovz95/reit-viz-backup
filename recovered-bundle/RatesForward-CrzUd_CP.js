...
function ke(n) {
  ...
  let W = 0,
    i = "—";
  k != null && (W = Math.max(-100, Math.min(100, -k * 200)), i =
    `${k>=0?"+":""}${(k*100).toFixed(0)}bp over 30d`);
  let u = 0,
    g = "—";
  if (f != null && L != null) {
    const C = Math.max(-50, Math.min(60, L * 300)),
      b = Math.max(-30, Math.min(40, (f - .5) * 80));
    u = C + b, u = Math.max(-100, Math.min(100, u)), g =
      `TP ${Z(f)} (${L>=0?"+":""}${(L*100).toFixed(0)}bp 30d)`
  }
  let y = 0,
    $ = "—";
  if (v != null && P != null) {
    ...
    y = C + b, y = Math.max(-100, Math.min(100, y)), $ =
      `2s10s ${Z(v)} (${P>=0?"+":""}${(P*100).toFixed(0)}bp 30d)`
  }
  const N = (R + W + u + y) / 4;
  let p = "Neutral";
  return N >= 50 ? p = "Convexity Activated" : N >= 20 ? p = "Convexity Building" : N >= -20 ? p =
    "Mixed / Neutral" : N >= -50 ? p = "Rates Risk-On" : p = "Convexity Suppressed", {
      score: N,
      regime: p,
      components: [{
        label: "Forward Path",
...
function ze() {
  const [n, w] = c.useState({}), [a, k] = c.useState([]), [f, L] = c.useState([]), [v, P] = c
    .useState([]), [R, O] = c.useState([]), [W, i] = c.useState([]), [u, g] = c.useState(!0), [y,
    $] = c.useState(null);
  c.useEffect(() => {
    let s = !1;
    return (async () => {
      try {
        g(!0);
        const [r, o, d, t, h, x, S] = await Promise.all([Promise.all(fe.map(M => U(M
          .id))), U("DGS2"), U("DGS10"), U("THREEFYTP10"), U("THREEFY10"), U(
            "THREEFF10"), U("VNQ").catch(() => [])
        ]);
        if (s) return;
        const z = {};
        fe.forEach((M, _) => {
  ...
  c.useEffect(() => {
    ...
    t.font = "11px ui-sans-serif, system-ui", t.fillStyle = "#a1a1aa", t.strokeStyle =
      "rgba(82, 82, 91, 0.15)", t.lineWidth = 1;
    const ne = 5;
    for (let l = 0; l <= ne; l++) {
      const G = X + l * (ee - X) / ne,
        ie = te(G);
      t.beginPath(), t.moveTo(h, ie), t.lineTo(h + j, ie), t.stroke(), t.textAlign = "right", t
        .textBaseline = "middle", t.fillText(`${G.toFixed(2)}%`, h - 6, ie)
    }
    ...
    for (const l of xe) {
      ...
      const G = A(l);
      t.fillText(l < 1 ? `${(l*12).toFixed(0)}M` : `${l}Y`, G, S + D + 6), t.beginPath(), t
        .moveTo(G, S + D), t.lineTo(G, S + D + 4), t.strokeStyle = "rgba(82, 82, 91, 0.4)", t
        .stroke(), t.strokeStyle = "rgba(82, 82, 91, 0.15)"
    }
    const ae = A(2);
    if (t.fillStyle = "rgba(245, 158, 11, 0.08)", t.fillRect(A(1), S, A(2) - A(1), D), t
      .strokeStyle = "rgba(245, 158, 11, 0.5)", t.setLineDash([4, 4]), t.beginPath(), t.moveTo(
        ae, S), t.lineTo(ae, S + D), t.stroke(), t.setLineDash([]), t.fillStyle = "#f59e0b", t
      .textAlign = "center", t.font = "10px ui-sans-serif, system-ui", t.fillText(
        "12-24m horizon", (A(1) + A(2)) / 2, S + 2), C != null) {
      const l = te(C);
      t.strokeStyle = "rgba(161, 161, 170, 0.6)", t.setLineDash([2, 4]), t.lineWidth = 1.2, t
        .beginPath(), t.moveTo(h, l), t.lineTo(h + j, l), t.stroke(), t.setLineDash([]), t
        .fillStyle = "#a1a1aa", t.font = "10px ui-sans-serif, system-ui", t.textAlign = "left",
        t.fillText(`Spot 3M ${C.toFixed(2)}%`, h + 6, l - 4)
    }
    const Ne = [{
      name: "90d ago",
    ...
    for (const l of Ne) l.data.length && (t.strokeStyle = l.color, t.lineWidth = l.lineWidth, l
      .dash ? t.setLineDash(l.dash) : t.setLineDash([]), t.beginPath(), l.data.forEach((G,
        ie) => {
          const Se = A(G.years),
            Me = te(G.forward);
    ...
    for (const l of Ne) t.strokeStyle = l.color, t.lineWidth = l.lineWidth, l.dash ? t
      .setLineDash(l.dash) : t.setLineDash([]), t.beginPath(), t.moveTo(de, pe + 6), t.lineTo(
        de + 18, pe + 6), t.stroke(), t.setLineDash([]), t.fillStyle = "#d4d4d8", t.fillText(l
        .name, de + 22, pe), de += 80
  }, [p, C]), c.useEffect(() => {
    const s = () => {
        if (J.current && p) {
  ...
  const we = E.score >= 50 ? "text-emerald-400" : E.score >= 20 ? "text-emerald-300" : E.score >= -
    20 ? "text-zinc-300" : E.score >= -50 ? "text-amber-400" : "text-rose-400";
  return e.jsx("div", {
      ...
      children: [e.jsxs("div", {
        ...
        children: [e.jsxs("div", {
          children: [e.jsx("h1", {
            ...
            children: [
              "Forward-expectations dashboard for REIT convexity activation. Latest data: ",
              N ?? "—"
            ]
          })]
        }), e.jsxs("div", {
        ...
        children: [e.jsxs("div", {
          ...
          children: [e.jsxs("div", {
            children: [e.jsx("h2", {
              ...
              className: "text-xs text-muted-foreground mt-0.5",
              children: [
                "Same composite formula applied weekly across the last ",
                F ? Math.round((Date.now() - new Date(F.startDate)
                  .getTime()) / (365 * 864e5)) : 10,
                " years. Score on right axis (green above 0 = convexity bias, red below = rates-selling-off). VNQ rebased to 100 on left axis (violet)."
              ]
            })]
          }), F && e.jsxs("div", {
            ...
            children: [e.jsxs("div", {
              ...
              children: [e.jsx("div", {
                ...
                className: "font-semibold text-foreground",
                children: [F.min >= 0 ? "+" : "", F.min
                  .toFixed(0), " to ", F.max >= 0 ? "+" :
                  "", F.max.toFixed(0)
                ]
              }), e.jsxs("div", {
                className: "text-[10px] text-muted-foreground",
                children: ["median ", F.p50 >= 0 ? "+" : "", F
                  .p50.toFixed(0)
                ]
              })]
            }), e.jsxs("div", {
              ...
              children: [e.jsx("div", {
                ...
                className: "font-semibold text-foreground",
                children: [F.p25 >= 0 ? "+" : "", F.p25
                  .toFixed(0), " to ", F.p75 >= 0 ? "+" :
                  "", F.p75.toFixed(0)
                ]
              }), e.jsxs("div", {
                className: "text-[10px] text-muted-foreground",
          ...
          children: [e.jsxs("div", {
            ...
            children: [e.jsxs("div", {
              ...
              children: [e.jsx("div", {
                ...
                children: Y.cuts12 != null ?
                  `${Y.cuts12>=0?"+":""}${Y.cuts12.toFixed(1)} cuts` :
                  "—"
              }), e.jsx("div", {
                className: "text-[10px] text-muted-foreground",
                children: Y.drop12bp != null ?
                  `${Y.drop12bp>=0?"−":"+"}${Math.abs(Y.drop12bp).toFixed(0)}bp` :
                  ""
              })]
            }), e.jsxs("div", {
              className: "px-3 py-1.5 rounded bg-muted/40 border border-border",
              children: [e.jsx("div", {
                ...
                children: [Y.cuts24 >= 0 ? "+" : "", Y.cuts24
                  .toFixed(1), " cuts"
                ]
              }), e.jsxs("div", {
                className: "text-[10px] text-muted-foreground",
                children: [Y.drop24bp >= 0 ? "−" : "+", Math
                  .abs(Y.drop24bp).toFixed(0), "bp"
                ]
              })]
            })]
          })]
          ...
          children: [e.jsxs("div", {
            ...
            children: [e.jsxs("div", {
              ...
              children: [e.jsx("div", {
                ...
                children: b?.d30 != null ?
                  `${b.d30>=0?"+":""}${(b.d30*100).toFixed(0)}bp` :
                  "—"
              })]
            }), e.jsxs("div", {
              className: "rounded bg-muted/40 border border-border px-2.5 py-1.5",
              children: [e.jsx("div", {
                ...
                children: b?.d90 != null ?
                  `${b.d90>=0?"+":""}${(b.d90*100).toFixed(0)}bp` :
                  "—"
              })]
            })]
          })]
          ...
          children: [e.jsxs("div", {
            ...
            children: [e.jsxs("div", {
              ...
              children: [e.jsx("div", {
                ...
                className: "text-[10px] text-muted-foreground",
                children: T.tp30 != null ?
                  `${T.tp30>=0?"+":""}${(T.tp30*100).toFixed(0)}bp 30d` :
                  ""
              })]
            }), e.jsxs("div", {
              className: "rounded bg-muted/40 border border-border px-2.5 py-1.5",
              children: [e.jsx("div", {
                ...
                className: "text-[10px] text-muted-foreground",
                children: T.exp30 != null ?
                  `${T.exp30>=0?"+":""}${(T.exp30*100).toFixed(0)}bp 30d` :
                  ""
              })]
            })]
          })]
        ...
        children: [e.jsxs("div", {
          ...
          children: [e.jsxs("div", {
            ...
            children: [e.jsxs("div", {
              ...
              children: [m.latest >= 0 ? "+" : "", (m.latest * 100)
                .toFixed(0), "bp"
              ]
            }), e.jsx("div", {
              className: "text-[11px] text-muted-foreground",
          ...
          children: [e.jsxs("div", {
            ...
            children: [e.jsx("div", {
              ...
              children: m?.d30 != null ?
                `${m.d30>=0?"+":""}${(m.d30*100).toFixed(0)}bp` :
                "—"
            })]
          }), e.jsxs("div", {
            className: "rounded bg-muted/40 border border-border px-2.5 py-1.5",
            children: [e.jsx("div", {
              ...
              children: m?.d90 != null ?
                `${m.d90>=0?"+":""}${(m.d90*100).toFixed(0)}bp` :
                "—"
            })]
          })]
        })]
        ...
        children: [e.jsx("div", {
          ...
          children: [
            "The forward Treasury curve is the single best forward-expectations measure, but for REIT convexity work specifically you need the 10Y decomposed into expectations and term premium — because Cluster 1's activation depends on the 10Y falling ",
            e.jsx("em", {
