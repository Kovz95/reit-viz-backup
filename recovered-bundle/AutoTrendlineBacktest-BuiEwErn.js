...
function Ge(o) {
  return Number.isFinite(o) ? o >= .7 ? "text-emerald-400 font-semibold" : o >= .6 ?
    "text-emerald-500" : o >= .5 ? "text-amber-400" : "text-red-400" : "text-muted-foreground"
}

function Ze(o) {
  return Number.isFinite(o) ? o > 0 ? "text-emerald-400" : o < 0 ? "text-red-400" :
...
function Jt() {
  ...
  const [b, Ne] = i.useState(10), [k, je] = i.useState(!1), [U, ve] = i.useState("5,8,10,15,20"), [
      l, we
    ] = i.useState("universe"), [E, Se] = i.useState("ABR"), [g, ie] = q(
      "auto-trendline-bt:input-selection", $t), [R, Ce] = i.useState("NEE"), [$, Re] = i.useState(
      "SO"), [A, $e] = i.useState([]), [N, Te] = i.useState("daily"), w = Bt(x, l === "pairCombo",
      "atl-paircombo"), [z, Be] = i.useState(5), [y, Me] = i.useState(252), [T, Ae] = i.useState(
    () => new Set(Z)), [u, Le] = i.useState("1M"), [L, Pe] = i.useState("hitRate"), [K, oe] = i
    .useState("desc"), [P, De] = i.useState(!1), [H, Ie] = i.useState(20), [W, _e] = i.useState(!1),
    [Q, Fe] = i.useState({
      current: 0,
      total: 0
    }), [le, Oe] = q("auto-trendline-bt:rows", []), [ce, Ue] = q("auto-trendline-bt:skipped", []), [
      Ee, Qe
    ] = q("auto-trendline-bt:lastRunAt", null), [B, Xe] = q("auto-trendline-bt:lastRunTimeframe",
      "daily"), J = i.useRef(!1), et = i.useCallback(() => ({
      n: b,
      sweepMode: k,
      nList: U,
      scopeMode: l,
      ...
      try {
        if (typeof e?.n == "number" && Ne(e.n), typeof e?.sweepMode == "boolean" && je(e
            .sweepMode), typeof e?.nList == "string" && ve(e.nList), (e?.scopeMode === "single" ||
            e?.scopeMode === "pair" || e?.scopeMode === "pairCombo" || e?.scopeMode ===
            "universe" || e?.scopeMode === "basket") && we(e.scopeMode), typeof e?.singleTicker ==
          "string" && Se(e.singleTicker), typeof e?.pairTickerA == "string" && Ce(e.pairTickerA),
          typeof e?.pairTickerB == "string" && Re(e.pairTickerB), Array.isArray(e
          ?.basketTickers) && $e(e.basketTickers), e?.pc && w.hydrate(e.pc), (e?.timeframe ===
            "daily" || e?.timeframe === "weekly") && Te(e.timeframe), typeof e?.minSignals ==
          "number" && Be(e.minSignals), typeof e?.minBars == "number" && Me(e.minBars), Array
          .isArray(e?.enabledKinds) && Ae(new Set(e.enabledKinds.filter(a => Z.includes(a)))),
          typeof e?.primaryHorizon == "string" && Le(e.primaryHorizon), typeof e?.sortKey ==
          "string" && Pe(e.sortKey), typeof e?.sortDir == "string" && oe(e.sortDir), typeof e
          ?.liveOnly == "boolean" && De(e.liveOnly), typeof e?.liveMaxBars == "number" && Ie(e
            .liveMaxBars), e?.inputSelection && typeof e.inputSelection == "object") {
  ...
  const h = i.useMemo(() => {
      ...
      return l === "pairCombo" ? w.pairs.map(a => ({
        ...
        pairA: a.a,
        pairB: a.b
      })) : l === "basket" ? A.map(s => s.toUpperCase().trim()).filter(Boolean).map(s => x.find(
        c => c.ticker === s) || {
        ticker: s,
        name: s
      }) : (o && o.length > 0 ? o : x) || []
        ...
        await Promise.all(de.map(async r => {
          if (!J.current) try {
            ...
            if (r.pairA && r.pairB) {
              ...
              S = n.indices.map(F => c[F] || ""), p = n.prices.slice(), ee = p
              .slice(), te = p.slice()
              ...
              if (!n || n.closes.length < y) {
                a.push({
                  ticker: r.ticker,
                  reason: n ?
                    `only ${n.closes.length} bars (need ${y}) for ${g.metric}` :
                    `no workbook data for ${g.metric}`
                });
                return
              }
              ...
              for (const j of Z) {
                ...
                for (const ae of G) {
                  ...
                  for (const he of Y) {
                    ...
                    const fe = (Je - re) / re;
                    C.push(fe), (ue === "long" && fe > 0 || ue === "short" && fe <
                      0) && xe++
                  }
                  ze[ae.label] = C.length > 0 ? xe / C.length : NaN, Ke[ae.label] = C
                    .length > 0 ? C.reduce((he, O) => he + O, 0) / C.length : NaN, He[
                      ae.label] = C.length > 0 ? Dt(C) : NaN
                }
    ...
    D = i.useMemo(() => {
      ...
      return e = e.slice().sort((s, c) => {
        ...
        return typeof m == "string" && typeof d == "string" ? a * m.localeCompare(d) : a * (
          m - d)
      }), e
    }, [le, T, P, H, L, K, u]),
    I = e => {
    ...
    it = i.useCallback(() => {
      ...
      const e = G.map(r => r.label),
        s = [
          ["ticker", "name", "n", "timeframe", "kind", "direction", "signals", "last_signal_date",
            "bars_since_last", ...e.flatMap(r => [`hit_${r}`, `avg_${r}`, `med_${r}`])
          ].join(",")
        ];
      for (const r of D) {
        const S = [r.ticker, JSON.stringify(r.name), String(r.n), B, r.kind, r.direction, String(r
          .count), r.lastSignalDate, String(r.barsSinceLast), ...e.flatMap(p => [Number
          .isFinite(r.hitRate[p]) ? r.hitRate[p].toFixed(4) : "", Number.isFinite(r
            .avgReturn[p]) ? r.avgReturn[p].toFixed(6) : "", Number.isFinite(r.medReturn[
          p]) ? r.medReturn[p].toFixed(6) : ""
        ])];
        s.push(S.join(","))
      }
...
`)], {
      ...
      d.download =
        `auto-trendline-backtest-${de}-${B}-${new Date().toISOString().slice(0,10)}.csv`, d
        .click(), URL.revokeObjectURL(m)
    }, [D, b, k, B]),
    _ = e => L !== e ? "" : K === "asc" ? " ↑" : " ↓",
    ot = l === "single" ? `Single (${h.length})` : l === "pair" ? `Pair ${R||"?"}/${$||"?"}` : l ===
    "pairCombo" ? `Pair combo (${h.length})` : l === "basket" ? `Basket (${h.length})` :
    `Universe ${f} / ${v}`;
  return t.jsxs("div", {
      ...
      children: [t.jsxs("div", {
        ...
        children: [t.jsxs("div", {
          children: [t.jsx("h1", {
            ...
            children: ["Port of DojiEmoji's ", t.jsx("span", {
              ...
              }), " indicator. Detects pivot fractals (period ", t
              .jsx("span", {
                className: "font-mono",
                children: "n"
              }),
              "), draws trendlines through the most recent two up/down pivots, and backtests close-vs-line crosses. Hit = direction matches forward-return sign. Horizons are always in ",
              t.jsx("span", {
                className: "font-mono",
        ...
        children: [t.jsxs("div", {
          ...
          children: [t.jsx("label", {
            ...
            children: ["single", "pair", "pairCombo", "basket",
              "universe"
            ].map(e => t.jsx("button", {
              onClick: () => we(e),
              ...
              title: e === "pairCombo" ?
                ...
                void 0,
              children: e === "single" ? "Single" : e === "pair" ?
                "Pair (A/B)" : e === "pairCombo" ? "Pair combo" :
                e === "basket" ? "Basket" : "Universe"
            }, e))
          })]
        }), t.jsxs("div", {
          className: "flex flex-col gap-0.5",
          ...
          children: [t.jsxs("label", {
            ...
            max: 50,
            value: b,
            onChange: e => Ne(Math.max(2, Math.min(50, Number(e.target
              .value) || 10))),
          ...
          children: [t.jsx("label", {
            ...
            max: 5e3,
            value: y,
            onChange: e => Me(Math.max(50, Number(e.target.value) ||
              252)),
          ...
          children: [t.jsx("input", {
            ...
            max: 500,
            value: H,
            onChange: e => Ie(Math.max(1, Number(e.target.value) ||
              20)),
        ...
        children: [t.jsxs("div", {
          children: ["Scope: ", t.jsx("span", {
            ...
            className: "ml-3",
            children: ["Last run: ", new Date(Ee).toLocaleTimeString(),
              " (", B, ")"
            ]
          }), ce.length > 0 && t.jsxs("span", {
            className: "ml-3 text-amber-400",
            title: ce.slice(0, 20).map(e => `${e.ticker}: ${e.reason}`)
              .join(`
`),
      ...
      children: D.length === 0 ? t.jsx("div", {
        ...
        children: W ? `Scanning ${Q.current} / ${Q.total}…` : le.length === 0 ? l ===
          "basket" && A.length === 0 ?
          "Add tickers to the basket, then click Run Scan." : l === "pairCombo" && h
          .length === 0 ?
          "Pick at least two legs in the pair-combo filter, then click Run Scan." :
          l === "pair" && h.length === 0 ?
          "Enter both A and B tickers, then click Run Scan." :
          "Configure and click Run Scan." : "No rows match the current filters."
      }) : t.jsxs("table", {
          ...
          children: D.map((e, a) => t.jsxs("tr", {
            ...
            children: [t.jsx("td", {
              ...
              children: t.jsx("button", {
                ...
                onClick: () => {
                  e.pairA && e.pairB ? Ye(e.pairA
                    .toUpperCase(), e.pairB.toUpperCase()) :
                    qe(e.ticker)
                },
                title: e.pairA && e.pairB ?
                  `Open ${e.pairA}/${e.pairB} ratio in Charts (Pairs preset)` :
                  "Open in Charts tab",
                children: e.ticker
              })
            }), t.jsx("td", {
              ...
              children: t.jsx("button", {
                ...
                title: e.pairA && e.pairB ?
                  `Open ${e.pairA}/${e.pairB} ratio in Charts (overlay skipped — ratio is unitless)` :
                  `Send auto-trendline overlay (n=${e.n}, ${B}) to Charts and open ${e.ticker}`,
                onClick: () => nt(e),
