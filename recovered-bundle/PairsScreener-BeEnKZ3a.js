...
var [Qe, et] = Ke(X), qt = Z.forwardRef((s, r) => {
  ...
  const {
    ...
    onChange: i,
    caller: X
  }), d = Z.useCallback(p => f((c = []) => [...c, p]), [f]), b = Z.useCallback(p => f((
  c = []) => c.filter(y => y !== p)), [f]);
  return e.jsx(Qe, {
    scope: s.__scopeToggleGroup,
    type: "multiple",
...
function os(s, r, o) {
  ...
  for (let c = 0; c < a; c++) n[c + 1] = n[c] + l[c], f[c + 1] = f[c] + i[c], d[c + 1] = d[c] + l[
    c] * l[c], b[c + 1] = b[c] + l[c] * i[c];
  ...
  for (let x = r; x < n; x++) {
    ...
    if (y) {
      const j = f[x],
        S = x - k;
      let L = !1;
      if (Number.isFinite(j) && (N === -1 && j <= 0 || N === 1 && j >= 0 || Math.abs(j) >= a) && (
          L = !0), !L && S >= i && (L = !0), !L && x === n - 1 && (L = !0), L) {
...
function vs() {
  const {
    ...
    totalCount: o,
    allTickers: a
  } = wt(), i = Ct(), [, l] = Mt(), [n, f] = h.useState("ols"), [d, b] = h.useState("universe"), [p,
      c
    ] = h.useState(() => At()), [y, k] = h.useState(""), [N, C] = h.useState(() => new Set), [A,
    M] = h.useState("workbook"), {
      metas: x
    } = Wt(), v = 50, j = 2e3, [S, L] = h.useState(ds), [P, me] = h.useState(ls), [U, Ae] = h
    .useState(1500), [I, Le] = h.useState(.1), [te, Fe] = h.useState(5), [se, Pe] = h.useState(60),
    [W, Te] = h.useState(1.5), [ae, Ve] = h.useState(!1), [E, Ie] = h.useState(.55), [q, Be] = h
    .useState("rolling"), [K, Ze] = h.useState(252), [J, Oe] = h.useState(!1), [O, Ge] = h
  .useState({
      current: 0,
      total: 0
    }), [Q, fe] = h.useState([]), xe = h.useRef(!1), [z, De] = h.useState("pValue"), [re, be] = h
    .useState("asc"), ct = h.useCallback(() => {
      const t = {
      ...
      if (t) {
        if ((t.model === "ols" || t.model === "ratio") && f(t.model), (t.scope === "universe" || t
            .scope === "pairCombo") && b(t.scope), t.pcFiltersSer && typeof t.pcFiltersSer ==
          "object") {
        ...
        typeof t.pcClassSearch == "string" && k(t.pcClassSearch), Array.isArray(t
            .pcManualTickersSer) && C(new Set(t.pcManualTickersSer)), typeof t.olsResidWindow ==
          "number" && L(t.olsResidWindow), typeof t.betaLookback == "number" && me(t
          .betaLookback), typeof t.verifyDays == "number" && Ae(t.verifyDays), typeof t.pMax ==
          "number" && Le(t.pMax), typeof t.hlMin == "number" && Fe(t.hlMin), typeof t.hlMax ==
          "number" && Pe(t.hlMax), typeof t.absZMin == "number" && Te(t.absZMin), typeof t
          .stableOnly == "boolean" && Ve(t.stableOnly), typeof t.minWinRate == "number" && Ie(t
            .minWinRate), (t.btResidMode === "rolling" || t.btResidMode === "insample") && Be(t
            .btResidMode), typeof t.btRollingWindow == "number" && Ze(t.btRollingWindow), t
          .sortKey && De(t.sortKey), t.sortDir && be(t.sortDir), Array.isArray(t.allResults) &&
          fe(t.allResults)
      }
    }, []), ut = _t();
  ...
  const ne = h.useMemo(() => p.economy.size + p.sector.size + p.subsector.size + p.industryGroup
      .size + p.industry.size + p.subindustry.size + N.size + (y.trim().length > 0 ? 1 : 0) === 0 ?
      [] : Ft(A === "global" ? x : a, p, y, N).map(g => g.ticker.toUpperCase()).filter((g, w, F) =>
        F.indexOf(g) === w), [a, x, A, p, y, N]),
  ...
  const mt = h.useCallback(async () => {
          ...
          w = await Promise.all(g.map(async ([F, D]) => {
            try {
              ...
              if (n === "ols") {
                ...
                const G = m.olsResidZ.length ? m.olsResidZ[m.olsResidZ.length - 1].value :
                  NaN,
                  T = ss(m.priceA, m.priceB, U),
                  ie = q === "rolling" ? os(m.priceA, m.priceB, K) : ns(m.priceA, m
                    .priceB),
                  Y = m.cointStats.halfLife,
                  Ne = Math.max(10, Math.min(60, Number.isFinite(Y) && Y > 0 ? Math.round(
                    Y * 4) : 60)),
    ...
    ee = h.useMemo(() => {
      const t = Q.filter(g => !(!Number.isFinite(g.pValue) || g.pValue > I || !Number.isFinite(g
            .halfLife) || g.halfLife < te || g.halfLife > se || !Number.isFinite(g.currentZ) ||
          Math.abs(g.currentZ) < W || ae && (!Number.isFinite(g.pValueRecent) || g
            .pValueRecent > I) || Number.isFinite(E) && E > 0 && (!Number.isFinite(g
            .backtestWinRate) || g.backtestN < 5 || g.backtestWinRate < E))),
        u = re === "asc" ? 1 : -1;
      return [...t].sort((g, w) => {
        switch (z) {
          case "pValue":
          ...
          case "ticker":
            return (g.tickerA + "/" + g.tickerB).localeCompare(w.tickerA + "/" + w.tickerB) *
              u;
          default:
            return 0
        }
    ...
    $ = h.useCallback(t => {
      t === z ? be(u => u === "asc" ? "desc" : "asc") : (De(t), be(t === "absZ" || t ===
        "hedgeRatio" || t === "backtestWinRate" ? "desc" : "asc"))
    }, [z]),
    H = t => z !== t ? e.jsx(zt, {
    ...
    ht = h.useCallback(() => {
      const u =
        `Ticker A,Ticker B,Model,Coint p (full),ADF (full),Coint p (recent),ADF (recent),Half-Life (d),Half-Life Recent (d),Hedge Ratio,${n==="ols"?"Current OLS Z":"Current Ratio Z"},Stable,BT Trades,BT Win%,BT Avg P&L (bps)`,
        R = ee.map(m => {
          const he = m.pValue <= I && m.pValueRecent <= I ? "YES" : "NO";
          return [m.tickerA, m.tickerB, n === "ols" ? "OLS" : "Ratio", m.pValue, m.adfStat, m
            .pValueRecent, Number.isFinite(m.adfStatRecent) ? m.adfStatRecent : "", m
            .halfLife, Number.isFinite(m.halfLifeRecent) ? m.halfLifeRecent : "Inf", m
            .hedgeRatio, Number.isFinite(m.currentZ) ? m.currentZ.toFixed(2) : "", he, m
            .backtestN, Number.isFinite(m.backtestWinRate) ? (m.backtestWinRate * 100)
            .toFixed(1) : "", Number.isFinite(m.backtestAvgPnL) ? (m.backtestAvgPnL * 1e4)
            .toFixed(1) : ""
          ].join(",")
        }),
        g = [u, ...R].join(`
`),
      ...
      D.href = F, D.download = `pair_screener_${new Date().toISOString().slice(0,10)}.csv`, D
        .click(), URL.revokeObjectURL(F)
    }, [ee, I, n]),
    pt = (t, u) => !Number.isFinite(t) || u < 5 ? "text-muted-foreground" : t >= .65 ?
    "text-emerald-400" : t >= .55 ? "text-green-400" : t >= .45 ? "text-yellow-400" :
    "text-red-400",
    gt = t => t <= .01 ? "text-emerald-400" : t <= .05 ? "text-green-400" : t <= .1 ?
    "text-yellow-400" : "text-muted-foreground",
    bt = t => {
      const u = Math.abs(t);
      return Number.isFinite(t) ? u >= 2.5 ? "text-red-400" : u >= 2 ? "text-orange-400" : u >=
        1.5 ? "text-yellow-400" : "text-muted-foreground" : "text-muted-foreground"
    };
  return e.jsxs("div", {
    ...
    children: [e.jsxs("div", {
      ...
      children: [e.jsxs("div", {
        ...
        children: [d === "universe" ? e.jsx(Ue, {
          ...
          children: [B.length, " tickers · ", oe.toLocaleString(), " pairs",
            d === "pairCombo" && oe > j && e.jsxs("span", {
      ...
      children: [e.jsx(_, {
          ...
          children: [e.jsx(de, {
            ...
            children: e.jsx("div", {
              children: e.jsx(_, {
                ...
                onChange: t => Ie(Math.max(0, Math.min(100, t)) /
                  100),
                step: 1,
                min: 0,
                max: 100,
        ...
        children: e.jsxs(ce, {
          children: [e.jsx(de, {
            ...
            className: "max-w-xs text-xs leading-relaxed",
            children: [
              "Show only pairs cointegrated at p ≤ pMax on BOTH the full sample AND the recent ",
              U,
              "-day window. Filters out pairs whose long-run cointegration is dragged down by stale historical regimes."
            ]
          })]
        })
      }), e.jsx("div", {
        ...
        children: e.jsxs(ce, {
          children: [e.jsx(de, {
            ...
            children: [e.jsx("strong", {
              ...
              }), ": β estimated at each bar using only the prior ", K,
              "-bar window — strict backward-looking, no look-ahead bias. Win rates and avg P&L are out-of-sample-honest.",
              e.jsx("br", {}), e.jsx("strong", {
      ...
      children: [e.jsxs("div", {
        ...
        children: [e.jsx(Bt, {
          ...
          className: "font-mono",
          children: ["Scanning ", O.current.toLocaleString(), " / ", O.total
            .toLocaleString(), " pairs"
          ]
        }), e.jsxs("span", {
          className: "font-mono text-foreground",
          children: [O.total > 0 ? (O.current / O.total * 100).toFixed(1) :
            "0.0", "%"
          ]
        })]
      }), e.jsx("div", {
      ...
      children: Q.length === 0 && !J ? e.jsx("div", {
        ...
        children: e.jsxs("div", {
          ...
          children: [e.jsx(Xe, {
            ...
            children: [
              "The screener iterates every unordered pair from the active Universe (currently",
              " ", e.jsx("span", {
                className: "font-mono text-foreground",
                children: B.length
              }), " ", "tickers · ", e.jsx("span", {
              ...
              }), " ", "pairs), runs an Engle-Granger ADF test on the",
              n === "ols" ?
              " OLS residuals (log A vs log B), with an estimated hedge ratio," :
              " raw log-ratio (log A − log B), forcing a 1:1 hedge,", " ",
              "and ranks pairs that are cointegrated, mean-revert quickly, and are currently dislocated.",
              e.jsx("br", {}), e.jsx("br", {}),
        ...
        children: [e.jsx("thead", {
          ...
          children: e.jsxs("tr", {
            ...
            children: [e.jsx("th", {
              ...
              children: e.jsxs("div", {
                className: "flex items-center gap-1",
                children: ["Hedge β", n === "ratio" && e.jsx(
                  "span", {
              ...
              children: e.jsxs("div", {
                className: "flex items-center gap-1",
                children: [n === "ols" ? "Current OLS Z" :
                  "Current Ratio Z", " ", H("absZ")
                ]
              })
            }), e.jsx("th", {
              className: "px-3 py-2 cursor-pointer hover:text-foreground",
              ...
              children: e.jsx(le, {
                ...
                children: e.jsxs(ce, {
                  children: [e.jsx(de, {
                    ...
                    className: "max-w-xs text-xs leading-relaxed",
                    children: [
                      "Real-P&L backtest: historical |Z| entries on the cointegrating series, with the same entry threshold, exits, and costs you set above. Format: ",
                      e.jsx("span", {
                        className: "font-mono",
                        children: "N · win%"
                      }), "."
          ...
          children: ee.length === 0 ? e.jsx("tr", {
            ...
            return e.jsxs("tr", {
              ...
              children: [e.jsxs("td", {
                ...
                children: Number.isFinite(t.halfLife) ? t.halfLife
                  .toFixed(1) : "∞"
              }), e.jsx("td", {
                ...
                children: Number.isFinite(t.currentZ) ? t.currentZ
                  .toFixed(2) : "—"
              }), e.jsx("td", {
                className: "px-3 py-1.5",
                children: t.backtestN > 0 && Number.isFinite(t
                  .backtestWinRate) ? e.jsxs("div", {
                  ...
                  children: [e.jsx("span", {
                      ...
                      children: [(t.backtestWinRate * 100)
                        .toFixed(0), "%"
                      ]
                    }), Number.isFinite(t.backtestAvgPnL) &&
                    e.jsxs("span", {
                      className: "text-muted-foreground/60 text-[10px]",
                      children: ["(", (t.backtestAvgPnL *
                        1e4).toFixed(0), "bps)"]
                    })
                  ]
                }) : e.jsx("span", {
                ...
                children: (() => {
                  const R = t.pValue <= I,
                    g = t.pValueRecent <= I,
                    w = Number.isFinite(t.pValueRecent) ? t
                    .pValueRecent.toFixed(2) : "—";
      ...
      children: [e.jsxs("span", {
        ...
        className: "text-muted-foreground/60",
        children: ["Model: ", n === "ols" ? "OLS Residual Z" :
          "Raw Log-Ratio (β=1)", " · OLS resid ", S, "d · β-lookback ", P,
          "d · MacKinnon CV (1%/5%/10% = -3.96/-3.37/-3.07)"
        ]
      })]
    })]
  })
